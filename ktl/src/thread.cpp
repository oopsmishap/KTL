...
{
thread_base::thread_base(internal_object_type thread_obj) : m_thread{thread_obj}
{
}
...
auto thread_base::set_priority(priority_t new_priority) const noexcept -> priority_t
{
...
auto thread_base::try_obtain_thread_object(HANDLE handle) noexcept -> internal_object_type
{
    void*          thread_obj;
    const NTSTATUS status{ObReferenceObjectByHandle(
        handle, THREAD_ALL_ACCESS, *PsThreadType, KernelMode, addressof(thread_obj), nullptr
    )};
...
system_thread::~system_thread() noexcept
{
    if (joinable())
    {
        crt::set_termination_context(
            {crt::BugCheckReason::DestroyingJoinableThread, get_id(), this_thread::get_id()}
        );
        terminate();
    }
}
...
{
    throw_exception_if_not<kernel_error>(joinable(), STATUS_INVALID_THREAD, "thread isn't joinable");
}

auto system_thread::create_thread_impl(thread_routine_t start, void* raw_args) -> internal_object_type
{
    OBJECT_ATTRIBUTES attrs;
    InitializeObjectAttributes(addressof(attrs), nullptr, OBJ_KERNEL_HANDLE, nullptr, nullptr);
    HANDLE         thread_handle;
    const NTSTATUS status{PsCreateSystemThread(
        addressof(thread_handle), THREAD_ALL_ACCESS, addressof(attrs), nullptr, nullptr, start, raw_args
    )};
    throw_exception_if_not<kernel_error>(NT_SUCCESS(status), status, "thread creation failed");
...
auto io_thread::create_thread_impl(void* io_object, thread_routine_t start, void* raw_args) -> internal_object_type
{
    OBJECT_ATTRIBUTES attrs;
    InitializeObjectAttributes(addressof(attrs), nullptr, OBJ_KERNEL_HANDLE, nullptr, nullptr);
    HANDLE         thread_handle;
    const NTSTATUS status{IoCreateSystemThread(
        io_object, addressof(thread_handle), THREAD_ALL_ACCESS, addressof(attrs), nullptr, nullptr, start, raw_args
    )};
    throw_exception_if_not<kernel_error>(NT_SUCCESS(status), status, "thread creation failed");
    auto thread_obj{try_obtain_thread_object(thread_handle)};
    ZwClose(thread_handle);
