...
void recursive_mutex::wait_impl(KMUTEX* mtx, const LARGE_INTEGER* timeout)
{
    KeWaitForSingleObject(
        ...
        false,
        const_cast<LARGE_INTEGER*>(timeout)
    ); // Indefinite waiting
}
...
shared_mutex::shared_mutex() : MyBase()
{
    ...
    throw_exception_if_not<kernel_error>(NT_SUCCESS(status), status, "initialization of ERESOURCE failed");
}
...
{
void spin_lock_policy<SpinlockType::DpcOnly>::lock(KSPIN_LOCK& target) const noexcept
{
    KeAcquireSpinLockAtDpcLevel(addressof(target));
}

bool spin_lock_policy<SpinlockType::DpcOnly>::try_lock(KSPIN_LOCK& target) const noexcept
{
    return KeTryToAcquireSpinLockAtDpcLevel(addressof(target));
}

void spin_lock_policy<SpinlockType::DpcOnly>::unlock(KSPIN_LOCK& target) const noexcept
{
    KeReleaseSpinLockFromDpcLevel(addressof(target));
}

void spin_lock_policy<SpinlockType::Mixed>::lock(KSPIN_LOCK& target) const noexcept
{
    m_prev_irql = KeAcquireSpinLockRaiseToDpc(addressof(target));
}

bool spin_lock_policy<SpinlockType::Mixed>::try_lock(KSPIN_LOCK& target) const noexcept
{
...
void spin_lock_policy<SpinlockType::Mixed>::unlock(KSPIN_LOCK& target) const noexcept
{
    KeReleaseSpinLock(addressof(target), m_prev_irql);
}

void queued_spin_lock_policy<SpinlockType::DpcOnly>::lock(KSPIN_LOCK& target, KLOCK_QUEUE_HANDLE& queue_handle) const noexcept
{
    KeAcquireInStackQueuedSpinLockAtDpcLevel(addressof(target), addressof(queue_handle));
}

void queued_spin_lock_policy<SpinlockType::DpcOnly>::unlock(KLOCK_QUEUE_HANDLE& queue_handle) const noexcept
{
    KeReleaseInStackQueuedSpinLockFromDpcLevel(addressof(queue_handle));
}

void queued_spin_lock_policy<SpinlockType::Mixed>::lock(KSPIN_LOCK& target, KLOCK_QUEUE_HANDLE& queue_handle) const noexcept
{
    KeAcquireInStackQueuedSpinLock(addressof(target), addressof(queue_handle));
}

void queued_spin_lock_policy<SpinlockType::Mixed>::unlock(KLOCK_QUEUE_HANDLE& queue_handle) const noexcept
{
    KeReleaseInStackQueuedSpinLock(addressof(queue_handle));
}
} // namespace th::details

semaphore::semaphore(counter_type start_count, counter_type upper_limit) noexcept : MyBase()
{
...
{
    return status == STATUS_TIMEOUT || status == STATUS_CANCELLED ? cv_status::timeout : cv_status::no_timeout;
}

NTSTATUS event_base::wait_impl(native_handle_type event, const LARGE_INTEGER* timeout) noexcept
{
