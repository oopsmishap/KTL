...
{
template <class RetType, class = void> struct coroutine_traits
{
};

template <class RetType> struct coroutine_traits<RetType, ktl::void_t<typename RetType::promise_type>>
{
    using promise_type = typename RetType::promise_type;
};
} // namespace coro::details

template <class RetType, class...> struct coroutine_traits : coro::details::coroutine_traits<RetType>
{
};
...
template <> struct coroutine_handle<void>
{
    ...
    [[nodiscard]] static constexpr coroutine_handle from_address(void* const ptr) noexcept
    { // strengthened
...
template <class _Promise> struct coroutine_handle
{
    constexpr coroutine_handle() noexcept = default;
    constexpr coroutine_handle(nullptr_t) noexcept
    {
    ...
    [[nodiscard]] static coroutine_handle from_promise(_Promise& from) noexcept
    { // strengthened
        const auto from_ptr  = const_cast<void*>(static_cast<const volatile void*>(ktl::addressof(from)));
    ...
    [[nodiscard]] _Promise& promise() const noexcept
    { // strengthened
        return *reinterpret_cast<_Promise*>(__builtin_coro_promise(m_ptr, 0, false));
    }
...
[[nodiscard]] constexpr bool operator==(const coroutine_handle<> lhs, const coroutine_handle<> rhs) noexcept
{
...
template <class Promise> struct hash<coroutine_handle<Promise>>
{
    [[nodiscard]] size_t operator()(const coroutine_handle<Promise>& coro) const noexcept
    {
...
template <> struct coroutine_handle<noop_coroutine_promise>
{
    ...
    {
        return *reinterpret_cast<noop_coroutine_promise*>(__builtin_coro_promise(m_ptr, 0, false));
    }
