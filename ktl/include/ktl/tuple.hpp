...
{
template <class Ty, class... Tys> struct count_entries
{
    // MSVC 19.20+
    static constexpr size_t value = (static_cast<size_t>(is_same_v<Ty, Tys>) + ...);
};

template <class Ty, class... Tys> struct is_present_once
{
    static constexpr bool value = count_entries<Ty, Tys...>::value == 1;
};

template <class Ty, class... Tys> struct is_present
{
    static constexpr bool value = count_entries<Ty, Tys...>::value > 0;
};

template <size_t Idx, class Ty, class Ty1, class... Tys> struct index_of_impl
{
    static constexpr size_t value = is_same_v<Ty, Ty1> ? Idx : index_of_impl<Idx + 1, Ty, Tys...>::value;
};

template <size_t Idx, class Ty, class Ty1> struct index_of_impl<Idx, Ty, Ty1>
{
    static constexpr size_t value = is_same_v<Ty, Ty1> ? Idx : -1;
};

template <class Ty, class... Tys> struct index_of
{
    static constexpr size_t value = index_of_impl<0, Ty, Tys...>::value;
};

template <size_t Idx, class Ty> struct tuple_element
{
    Ty value{};
};

template <size_t Idx, template <size_t, class> class TupleElement, class Ty>
constexpr auto extract_type(TupleElement<Idx, Ty>& element) noexcept -> decltype(element.value);

template <size_t Idx, class Ty> constexpr Ty& get_value(tuple_element<Idx, Ty>& element)
{
    return element.value;
}

template <size_t Idx, class Ty> constexpr const Ty& get_value(const tuple_element<Idx, Ty>& element)
{
    return element.value;
}

template <size_t Idx, class Ty> constexpr volatile Ty& get_value(volatile tuple_element<Idx, Ty>& element)
{
    return element.value;
}

template <size_t Idx, class Ty>
constexpr const volatile Ty& get_value(const volatile tuple_element<Idx, Ty>& element)
{
    return element.value;
}
...
template <class IndexType, class... Types> class tuple_base
{
};

template <size_t... Indices, class... Types>
class tuple_base<index_sequence<Indices...>, Types...> : public tuple_element<Indices, Types>...
{
  public:
    template <size_t Idx, class U> using MyElementBase = tuple_element<Idx, U>;

  public:
    explicit constexpr tuple_base() noexcept(conjunction_v<is_nothrow_default_constructible<Types>...>)
    {
    }
    ...
    template <
        ...
        enable_if_t<conjunction_v<is_copy_constructible<U>, is_copy_constructible<Types>...>, int> = 0>
    constexpr tuple_base(const Types&... args) noexcept(conjunction_v<is_nothrow_copy_constructible<Types>...>)
    ...
    template <
        class... OtherTypes,
        enable_if_t<sizeof...(OtherTypes) == sizeof...(Types) && conjunction_v<is_constructible<Types, OtherTypes&&>...>, int> = 0>
    constexpr tuple_base(OtherTypes&&... args
    ) noexcept(conjunction_v<is_nothrow_constructible<Types, OtherTypes>...>)
    ...
    template <
        class... OtherTypes,
        enable_if_t<sizeof...(OtherTypes) == sizeof...(Types) && conjunction<is_constructible<Types, OtherTypes>...>, int> = 0>
    ...
    constexpr tuple_base(const pair<U1, U2>& pair) noexcept(
        noexcept(is_nothrow_constructible_v<decltype(extract_type<0>(*this)), add_const_t<add_lvalue_reference_t<U1>>> && is_nothrow_constructible_v<decltype(extract_type<1>(*this)), add_lvalue_reference_t<U2>>)
    )
    ...
    constexpr tuple_base(pair<U1, U2>&& pair) noexcept(
        noexcept(is_nothrow_constructible_v<decltype(extract_type<0>(*this)), add_rvalue_reference_t<U1>> && is_nothrow_constructible_v<decltype(extract_type<1>(*this)), add_rvalue_reference_t<U2>>)
    )
    ...
    template <class... OtherTypes> constexpr tuple_base& operator=(const tuple_base<OtherTypes...>& other)
    {
    ...
    template <class... OtherTypes> constexpr tuple_base& operator=(tuple_base<OtherTypes...>&& other)
    {
        if (addressof(this) != other)
        {
    ...
    template <class U1, class U2> constexpr tuple_base& operator=(const pair<U1, U2>& pair)
    {
        return assign(make_index_sequence<2>{}, pair.first, pair.second);
    }

    template <class U1, class U2> constexpr tuple_base& operator=(pair<U1, U2>&& pair)
    {
        return assign(make_index_sequence<2>{}, move(pair.first), move(pair.second));
    }

    constexpr void swap(tuple_base& other) noexcept(conjunction_v<is_nothrow_swappable<Types>...>)
    {
  ...
  private:
    ...
    template <size_t... Indices, class... Types> tuple_base& assign_impl(Types&&... args)
    {
...
template <class... Types> class tuple : public tt::details::tuple_base<index_sequence_for<Types...>, Types...>
{
  public:
    using MyBase = tt::details::tuple_base<index_sequence_for<Types...>, Types...>;
...
template <class... Types> struct tuple_size;
...
template <class Ty> inline constexpr size_t tuple_size_v = tuple_size<Ty>::value;
...
#define KTL_GET_BY_IDX_IMPL(Const, Volatile, Ref, Move)                                     \
    ...
    {                                                                                       \
        static_assert(Idx < tuple_size_v<tuple<Types...>>, "tuple index is out of bounds"); \
...
#define KTL_GET_BY_TYPE_IMPL(Const, Volatile, Ref, Move)                                                    \
    ...
    constexpr decltype(auto) get(Const Volatile tuple<Types...> Ref target) noexcept                        \
    {                                                                                                       \
        static_assert(tt::details::is_present<Ty, Types...>::value, "Type Ty must be present in tuple");    \
        static_assert(                                                                                      \
            tt::details::is_present_once<Ty, Types...>::value, "Type Ty must be present in tuple only once" \
        );                                                                                                  \
...
template <size_t Idx, class Ty> using tuple_element = remove_reference<decltype(get<Idx>(declval<Ty>()))>;
...
template <class... Types>
void swap(tuple<Types...>& lhs, tuple<Types...>& rhs) noexcept(is_nothrow_swappable_v<tuple<Types...>>)
{
    lhs.swap(rhs);
}
...
namespace tt::details
{
template <class Ty> struct unwrap_refwrapper
{
    using type = Ty;
};
...
template <class Ty> using unwrap_refwrapper_t = typename unwrap_refwrapper<Ty>::type;

template <class Ty> using unwrap_decay_t = unwrap_refwrapper_t<decay_t<Ty>>;
} // namespace tt::details

template <class... Types> constexpr tuple<tt::details::unwrap_decay_t<Types>...> make_tuple(Types&&... args)
{
...
struct ignore_tag
{
    template <typename Ty> const ignore_tag& operator=(const Ty&) const noexcept
    {
        return *this;
    }
};
...
template <class... Types> [[nodiscard]] constexpr tuple<Types&...> tie(Types&... args) noexcept
{
    return tuple<Types&...>(args...);
}

template <class... Types> [[nodiscard]] constexpr tuple<Types&&...> forward_as_tuple(Types&&... args) noexcept
{
...
constexpr bool equal_to_impl(const LhsTuple& lhs, const RhsTuple& rhs, index_sequence<Indices...>)
{
    return ((get<Indices>(lhs) == get<Indices>(rhs)) && ...);
}

template <class... LhsTypes, class... RhsTypes>
constexpr bool equal_to(const tuple<LhsTypes...>& lhs, const tuple<RhsTypes...>& rhs)
{
...
template <class... LhsTypes, class... RhsTypes>
constexpr bool operator==(const tuple<LhsTypes...>& lhs, const tuple<RhsTypes...>& rhs)
{
...
constexpr bool less_impl(const tuple<LhsTypes...>& lhs, const tuple<RhsTypes...>& rhs);

template <size_t Idx, class... LhsTypes, class... RhsTypes>
constexpr bool less_helper(const tuple<LhsTypes...>& lhs, const tuple<RhsTypes...>& rhs)
{
...
template <size_t Idx, class... LhsTypes, class... RhsTypes>
constexpr bool
less_impl([[maybe_unused]] const tuple<LhsTypes...>& lhs, [[maybe_unused]] const tuple<RhsTypes...>& rhs)
{
    if constexpr (sizeof...(LhsTypes) >= sizeof...(RhsTypes) && Idx == sizeof...(RhsTypes))
    {
        return false;
    }
...
template <class... LhsTypes, class... RhsTypes>
constexpr bool less(const tuple<LhsTypes...>& lhs, const tuple<RhsTypes...>& rhs)
{
...
template <class F, class Tpl> constexpr decltype(auto) apply(F&& f, Tpl&& tpl)
{
    using idxs_t = make_index_sequence<tuple_size_v<remove_reference_t<Tpl>>>;
