...
template <class Ty, size_t Size> struct array
{
    ...
    [[nodiscard]] constexpr Ty* data() noexcept
    {
        return m_elems;
    }
    [[nodiscard]] constexpr const Ty* data() const noexcept
    {
        return m_elems;
    }
...
{
template <class First, class... Rest> struct enforce_same
{
    static_assert(
        (is_same_v<First, Rest> && ...),
...
template <class First, class... Rest> using enforce_same_t = typename enforce_same<First, Rest...>::type;
} // namespace cont::details

template <class First, class... Rest>
array(First, Rest...) -> array<cont::details::enforce_same_t<First, Rest...>, 1 + sizeof...(Rest)>;

template <class Ty> struct array<Ty, 0>
{
    using value_type      = Ty;
    using size_type       = size_t;
    using difference_type = ptrdiff_t;
    ...
    {
        crt_assert_with_msg(false, "operator[] call is invalid: array<Ty, 0> has no elements");
        return m_elems[0];
    }
    ...
    {
        crt_assert_with_msg(false, "front() call is invalid: array<Ty, 0> has no elements");
        return m_elems[0];
    }
    ...
    {
        crt_assert_with_msg(false, "back() call is invalid: array<Ty, 0> has no elements");
        return m_elems[0];
    }
...
template <class Ty, size_t Size, enable_if_t<Size == 0 || is_swappable_v<Ty>, int> = 0>
void swap(array<Ty, Size>& lhs, array<Ty, Size>& rhs) noexcept(is_nothrow_swappable_v<array<Ty, Size>>)
{
    return lhs.swap(rhs);
}

template <class Ty, size_t Size>
[[nodiscard]] bool operator==(const array<Ty, Size>& lhs, const array<Ty, Size>& rhs)
{
...
#define KTL_ARRAY_GET_IMPL(Const, Volatile, Ref, Move)                               \
    ...
    constexpr decltype(auto) get(Const Volatile array<Ty, Size> Ref target) noexcept \
    {                                                                                \
...
#define KTL_TO_ARRAY_IMPL(Ref, Move, Trait, Msg)                                                                  \
    ...
    [[nodiscard]] constexpr array<remove_cv_t<Ty>, Size> to_array_impl(Ty(Ref arr)[Size], index_sequence<Idx...>) \
    {                                                                                                             \
        return {{Move(arr[Idx])...}};                                                                             \
    }                                                                                                             \
    }                                                                                                             \
                                                                                                                  \
    template <class Ty, size_t Size, size_t... Idx>                                                               \
    [[nodiscard]] constexpr array<remove_cv_t<Ty>, Size> to_array(Ty(Ref arr)[Size])                              \
    {                                                                                                             \
        static_assert(                                                                                            \
            !is_array_v<Ty>,                                                                                      \
        ...
        static_assert(Trait, "N4830 [array.creation]/1: to_array requires " #Msg " constructible elements");      \
        return cont::details::to_array_impl(Move(arr), make_index_sequence<Size>{});                              \
    }
