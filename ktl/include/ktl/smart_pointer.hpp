...
{
template <class Ty> struct default_delete
{
...
template <class Ty> struct default_delete<Ty[]>
{
    using enable_delete_null = true_type; // calling operator()(nullptr) is valid
...
template <class Ty, class Deleter = mm::details::default_delete<Ty>> class unique_ptr
{
  public:
  ...
  public:
    template <
        class Dx = Deleter,
        enable_if_t<
            !is_pointer_v<Dx> && // Нельзя хранить nullptr
    ...
    constexpr explicit unique_ptr(pointer ptr) noexcept : m_value{zero_then_variadic_args{}, ptr}
    {
    }

    template <class Dx = Deleter, enable_if_t<is_constructible_v<Dx, const Dx&>, int> = 0>
    constexpr unique_ptr(pointer ptr, const Deleter& deleter) noexcept(is_nothrow_copy_constructible_v<Dx>)
        : m_value{deleter, ptr}
    {
    }

    // Dx isn't a reference type
    template <class Dx = Deleter, enable_if_t<!is_reference_v<Dx> && is_move_constructible_v<Dx>, int> = 0>
    constexpr unique_ptr(pointer ptr, Deleter&& deleter) noexcept(is_nothrow_move_constructible_v<Dx>)
        : m_value{move(deleter), ptr}
    {
    }

    // Dx is a reference type
    template <class Dx = Deleter, enable_if_t<is_reference_v<Dx> && is_constructible_v<Dx, remove_reference_t<Dx>>, int> = 0>
    constexpr unique_ptr(pointer ptr, remove_reference_t<Dx>&& deleter) = delete; // Preventing unwanted assignments

    template <class Dx = Deleter, enable_if_t<is_move_constructible_v<Dx>, int> = 0>
    constexpr unique_ptr(unique_ptr&& other) noexcept(is_nothrow_move_constructible_v<Dx>)
    ...
    template <
        ...
        class OtherDeleter,
        enable_if_t<
            !is_array_v<OtherTy> && is_convertible_v<typename unique_ptr<OtherTy, OtherDeleter>::pointer, pointer>
                && conditional_t<
            ...
            int> = 0>
    constexpr unique_ptr(unique_ptr<OtherTy, OtherDeleter>&& other
    ) noexcept(is_nothrow_constructible_v<deleter_type, OtherDeleter>)
        : m_value{forward<deleter_type>(other.get_deleter()), other.release()}
    {
    }

    template <class Dx = Deleter, enable_if_t<is_move_assignable_v<Dx>, int> = 0>
    constexpr unique_ptr& operator=(unique_ptr&& other) noexcept(is_nothrow_move_assignable_v<Dx>)
    {
    ...
    template <
        ...
        enable_if_t<
            ...
            int> = 0>
    constexpr unique_ptr& operator=(unique_ptr<OtherTy, OtherDeleter>&& other
    ) noexcept(is_nothrow_assignable_v<deleter_type, OtherDeleter>)
    {
  ...
  private:
    constexpr pointer& get_ref_to_ptr()
    {
        return m_value.get_second();
    }
...
constexpr bool operator==(const unique_ptr<Ty1, Dx1>& lhs, const unique_ptr<Ty2, Dx2>& rhs) noexcept
{
    using common_t =
        common_type_t<typename unique_ptr<Ty1, Dx1>::pointer, typename unique_ptr<Ty2, Dx2>::pointer>;
    return equal_to<common_t>{}(static_cast<common_t>(lhs.get()), static_cast<common_t>(rhs.get()));
}
...
constexpr bool operator<(const unique_ptr<Ty1, Dx1>& lhs, const unique_ptr<Ty2, Dx2>& rhs) noexcept
{
    using common_t =
        common_type_t<typename unique_ptr<Ty1, Dx1>::pointer, typename unique_ptr<Ty2, Dx2>::pointer>;
    return less<common_t>{}(static_cast<common_t>(lhs.get()), static_cast<common_t>(rhs.get()));
}
...
template <class Ty, class Dx> constexpr bool operator==(const unique_ptr<Ty, Dx>& ptr, nullptr_t) noexcept
{
    return !ptr;
}
...
template <class Ty, class Dx> constexpr bool operator==(nullptr_t, const unique_ptr<Ty, Dx>& ptr) noexcept
{
    return !ptr;
}
...
template <class Ty, class Dx> constexpr bool operator<(const unique_ptr<Ty, Dx>& ptr, nullptr_t null) noexcept
{
...
template <class Ty, class Dx> constexpr bool operator<(nullptr_t null, const unique_ptr<Ty, Dx>& ptr) noexcept
{
    return ptr < null;
}
...
constexpr bool operator<=(const unique_ptr<Ty, Dx>& ptr, nullptr_t null) noexcept
{
    return !(ptr > null);
}

template <class Ty, class Dx>
constexpr bool operator<=(nullptr_t null, const unique_ptr<Ty, Dx>& ptr) noexcept
{
    return !(null > ptr);
}
...
template <class Ty, class Dx> constexpr Ty* get_pointer(const unique_ptr<Ty, Dx>& ptr) noexcept
{
    return ptr.get();
}

// C++17 deduction guide
template <class Ty> unique_ptr(Ty*) -> unique_ptr<Ty>;

// C++17 deduction guide
template <class Ty, class Dx> unique_ptr(Ty*, Dx&&) -> unique_ptr<Ty, Dx>;
...
template <class Ty, class... Types> unique_ptr<Ty> make_unique(Types&&... args)
{
...
class ref_counter_base
{
  ...
  private:
    counter_t inc_strong_refs() noexcept
    {
        return ++m_strong_counter;
    }
    counter_t inc_weak_refs() noexcept
    {
        return ++m_weak_counter;
    }
    counter_t dec_strong_refs() noexcept
    {
        return --m_strong_counter;
    }
    counter_t dec_weak_refs() noexcept
    {
        return --m_weak_counter;
    }
...
class ref_counter_with_storage : public ref_counter_base, public compressed_pair<First, Second>
{
  public:
  ...
  private:
    ...
    {
        accessor::delete_this(get_context());
    }
...
template <class Ty, class Deleter, class DestroyObjectPolicy, class DeleteItselfPolicy>
class ref_counter_with_deleter
    : public ref_counter_with_storage<Deleter, Ty*, ref_counter_with_deleter<Ty, Deleter, DestroyObjectPolicy, DeleteItselfPolicy>>
{
  public:
    using MyBase =
        ref_counter_with_storage<Deleter, Ty*, ref_counter_with_deleter<Ty, Deleter, DestroyObjectPolicy, DeleteItselfPolicy>>;
    using deleter_type = Deleter;

  public:
    constexpr ref_counter_with_deleter() = default;

    template <class Dx = Deleter, enable_if_t<is_default_constructible_v<Dx>, int> = 0>
    ref_counter_with_deleter(Ty* ptr) noexcept(is_nothrow_default_constructible_v<Dx>)
        : MyBase(zero_then_variadic_args{}, ptr)
    {
    }

    template <class Dx>
    ref_counter_with_deleter(Ty* ptr, Dx&& deleter) noexcept(is_nothrow_constructible_v<Deleter, Dx>)
    ...
    Ty* get_ptr() noexcept
    {
        return MyBase::get_second();
    }
    deleter_type& get_deleter() noexcept
    {
        return MyBase::get_first();
    }
...
template <class Ty, class Deleter, class Alloc, class DestroyObjectPolicy, class DeleteItselfPolicy>
class ref_counter_with_deleter_and_alloc
    : public ref_counter_with_storage<
          Alloc,
          compressed_pair<Deleter, Ty*>,
          ref_counter_with_deleter_and_alloc<Ty, Deleter, Alloc, DestroyObjectPolicy, DeleteItselfPolicy>>
{
  public:
  ...
  public:
    ...
    template <
        class Dx                                                                            = Deleter,
        class Alc                                                                           = Alloc,
        enable_if_t<is_default_constructible_v<Dx> && is_default_constructible_v<Alc>, int> = 0>
    ref_counter_with_deleter_and_alloc(Ty* ptr
    ) noexcept(is_nothrow_default_constructible_v<Dx> && is_nothrow_default_constructible_v<Alc>)
        : MyBase(zero_then_variadic_args{}, storage_type{zero_then_variadic_args{}, ptr})
    {
    }

    template <class Dx, class Alc>
    ref_counter_with_deleter_and_alloc(
        Ty* ptr, Dx&& deleter, Alc&& alloc
    ) noexcept(is_nothrow_constructible_v<Deleter, Dx> && is_nothrow_constructible_v<Alloc, Alc>)
        : MyBase(forward<Alc>(alloc), storage_type{one_then_variadic_args{}, forward<Dx>(deleter), ptr})
    {
    }
    ...
    Ty* get_ptr() noexcept
    {
        return MyBase::get_second().get_second();
    }
...
{
    template <class Ty, class Deleter> static void Apply(Ty* target, Deleter& deleter)
    {
...
{
    template <class Ty, class Deleter> static void Apply(Ty* target, [[maybe_unused]] Deleter& deleter)
    {
        if (target)
        {
            destroy_at(target);
...
{
    template <class Ty, class Alloc> static void Apply(Ty* target, Alloc& alloc)
    {
        if (target)
        {
...
struct DeleteItself
{
    ...
    static void Apply(RefCounter* ref_counter, [[maybe_unused]] Deleter& deleter)
    {
        delete ref_counter;
    }
};
...
struct DeallocateItself
{
    ...
    static void Apply(RefCounter* ref_counter, Alloc& allocator)
    {
        ...
        allocator_traits<Alloc>::deallocate_bytes(alloc, ref_counter, sizeof(RefCounter));
    }
};
...
struct DestroyItselfAndDeallocateAll
{
    ...
    static void Apply(RefCounter* ref_counter, Alloc& allocator)
    {
        auto alloc{move(allocator)};
        destroy_at(ref_counter);
        allocator_traits<Alloc>::deallocate_bytes(alloc, ref_counter, sizeof(pair<RefCounter, Ty>));
    }
};
...
class refcounted_ptr_base
{
  ...
  protected:
    template <class U, class OtherPtr> friend class refcounted_ptr_base;

    constexpr refcounted_ptr_base() = default;

    explicit constexpr refcounted_ptr_base(element_type* value_ptr, ref_counter_base* ref_counter) noexcept
        : m_value_ptr{value_ptr}
        , m_ref_counter{ref_counter}
    {
    }

    constexpr Ty* get_value_ptr() const noexcept
    {
        return m_value_ptr;
    }
    ...
    template <class U> ConcretePtr& construct_from(U* ptr, ref_counter_base* ref_counter) noexcept
    {
    ...
    ConcretePtr& copy_construct_from(const refcounted_ptr_base<U, OtherPtr>& other) noexcept
    {
    ...
    template <class U, class OtherPtr>
    ConcretePtr& move_construct_from(refcounted_ptr_base<U, OtherPtr>&& other) noexcept
    {
        static_assert(is_convertible_v<U*, Ty*>, "can't convert value pointer");
        decref_if_not_null();
...
template <class Ty> class weak_ptr : public mm::details::refcounted_ptr_base<Ty, weak_ptr<Ty>>
{
  public:
  ...
  public:
    ...
    template <class OtherTy, enable_if_t<is_convertible_v<Ty*, OtherTy*>, int> = 0>
  ...
  protected:
    ...
    {
        MyBase::get_ref_counter()->Follow();
    }
    void decref() noexcept
    {
        MyBase::get_ref_counter()->Unfollow();
    }
};
...
template <class Ty> class shared_ptr : public mm::details::refcounted_ptr_base<Ty, shared_ptr<Ty>>
{
  public:
  ...
  private:
    template <class ControlBlock> using placeholder_type = pair<ControlBlock, Ty>;
  ...
  public:
    ...
    explicit shared_ptr(Ty* ptr) noexcept : MyBase(ptr, make_default_ref_counter(ptr, is_array<Ty>{}))
    {
    }

    template <class U, enable_if_t<is_convertible_v<U*, Ty*>, int> = 0>
    shared_ptr(U* ptr) noexcept : MyBase(ptr, make_default_ref_counter(ptr, is_array<Ty>{}))
    {
    }
    ...
    template <class U, class Alloc, enable_if_t<is_convertible_v<U*, Ty*>, int> = 0>
    shared_ptr(U* ptr, Alloc&& alloc, custom_allocator_tag_t)
        : MyBase(ptr, allocate_default_ref_counter(ptr, forward<Alloc>(alloc), is_array<Ty>{}))
    {
    }
    ...
    template <class U, enable_if_t<is_convertible_v<U*, Ty*>, int> = 0> shared_ptr(const weak_ptr<U>& wptr)
    {
    ...
    template <class U, class Dx> shared_ptr& operator=(unique_ptr<U, Dx>&& uptr)
    {
    ...
    template <class U, class Dx> void reset(U* ptr, Dx&& deleter) noexcept
    {
  ...
  protected:
    template <class ValTy, class... Types> friend shared_ptr<ValTy> make_shared(Types&&... args);
    ...
    void incref() noexcept
    {
        MyBase::get_ref_counter()->Acquire();
    }
    void decref() noexcept
    {
        MyBase::get_ref_counter()->Release();
    }

    shared_ptr(Ty* ptr, ref_counter_base* ref_counter) noexcept : MyBase(ptr, ref_counter)
    {
    }
  ...
  private:
    template <class U> static ref_counter_base* make_default_ref_counter(U* ptr, true_type)
    {
        return make_ref_counter(unique_ptr<U[]>{ptr}, mm::details::default_delete<U[]>{});
    }

    template <class U> static ref_counter_base* make_default_ref_counter(U* ptr, false_type)
    {
        return make_ref_counter(unique_ptr<U>{ptr}, mm::details::default_delete<U>{});
    }

    template <class U, class Dx, class Deleter>
    static ref_counter_base* make_ref_counter(unique_ptr<U, Dx>&& ptr_guard, Deleter&& dx)
    {
        ...
        {
            ref_counter =
                new ref_counter_with_deleter<U, remove_reference_t<Deleter>, DestroyObjectWithDeleter, DeleteItself>(
    ...
    static ref_counter_base* allocate_default_ref_counter(U* ptr, Alloc&& alloc, true_type)
    {
        return allocate_ref_counter_impl(
            unique_ptr<U[]>{}, ptr, forward<Alloc>(alloc), mm::details::default_delete<U[]>{}
        );
    }

    template <class U, class Alloc>
    static ref_counter_base* allocate_default_ref_counter(U* ptr, Alloc&& alloc, false_type)
    {
        return allocate_ref_counter_impl(
            unique_ptr<U>{}, ptr, forward<Alloc>(alloc), mm::details::default_delete<U>{}
        );
    ...
    static ref_counter_base* allocate_ref_counter_impl(Guard&& value_guard, U* ptr, Alloc&& alloc, Deleter&& dx)
    {
        ...
        using ref_counter_t = mm::details::ref_counter_with_deleter_and_alloc<
            U,
            remove_reference_t<Deleter>,
            remove_reference_t<Alloc>,
            mm::details::DestroyObjectWithDeleter,
            mm::details::DeallocateItself>;
        constexpr auto REF_COUNTER_SIZE{static_cast<size_type>(sizeof(ref_counter_t))};

        value_guard.reset(ptr);
        auto* ref_counter{
            reinterpret_cast<ref_counter_t*>(allocator_traits_type::allocate_bytes(alloc, REF_COUNTER_SIZE))
        };
        unique_ptr ref_counter_guard{
            ref_counter,
            [&alloc, REF_COUNTER_SIZE](ref_counter_t* target)
            {
                allocator_traits_type::deallocate_bytes(alloc, target, REF_COUNTER_SIZE);
            }
        };
...
template <class Ty> weak_ptr<Ty>::weak_ptr(const shared_ptr<Ty>& sptr) noexcept
{
...
namespace mm::details
{
template <class Ty> struct shared_from_weak : shared_ptr<Ty>
{
...
template <class Ty> shared_ptr<Ty> weak_ptr<Ty>::lock() const noexcept
{
    return mm::details::shared_from_weak<Ty>{}(*this);
}

template <class Ty1, class Ty2>
constexpr bool operator==(const shared_ptr<Ty1>& lhs, const shared_ptr<Ty2>& rhs) noexcept
{
    using common_t = common_type_t<typename shared_ptr<Ty1>::pointer, typename shared_ptr<Ty2>::pointer>;
    return equal_to<common_t>{}(static_cast<common_t>(lhs.get()), static_cast<common_t>(rhs.get()));
}
...
template <class Ty> constexpr bool operator==(const shared_ptr<Ty>& ptr, nullptr_t) noexcept
{
    return !ptr;
}
...
template <class Ty> constexpr bool operator==(nullptr_t, const shared_ptr<Ty>& ptr) noexcept
{
    return !ptr;
}
...
template <class Ty> constexpr bool operator<(const shared_ptr<Ty>& ptr, nullptr_t null) noexcept
{
    return less<const Ty*>{}(ptr.get(), null);
}
...
template <class Ty> constexpr bool operator<(nullptr_t null, const shared_ptr<Ty>& ptr) noexcept
{
...
template <class Ty> constexpr Ty* get_pointer(const shared_ptr<Ty>& ptr) noexcept
{
    return ptr.get();
}

// C++17 deduction guides
template <class Ty> shared_ptr(Ty*) -> shared_ptr<Ty>;

template <class Ty, class Dx> shared_ptr(Ty*, Dx&&, custom_deleter_tag_t) -> shared_ptr<Ty>;

template <class Ty, class Alloc> shared_ptr(Ty*, Alloc&&, custom_allocator_tag_t) -> shared_ptr<Ty>;

template <class Ty> shared_ptr(weak_ptr<Ty>) -> shared_ptr<Ty>;

template <class Ty, class Dx> shared_ptr(unique_ptr<Ty, Dx>) -> shared_ptr<Ty>;

template <class Ty, class... Types> shared_ptr<Ty> make_shared(Types&&... args)
{
    using ref_counter_t = mm::details::
        ref_counter_with_deleter<Ty, mm::details::default_delete<Ty>, mm::details::DestroyObjectInPlace, mm::details::DestroyItselfAndDeleteAll>;
    using placeholder_type         = typename shared_ptr<Ty>::template placeholder_type<ref_counter_t>;
    using placeholder_storage_type = aligned_storage_t<sizeof(placeholder_type), alignof(placeholder_type)>;
...
shared_ptr<Ty> allocate_shared(Alloc&& alloc, Types&&... args)
{
    ...
    static_assert(is_same_v<Ty, typename allocator_traits_type::value_type>, "Alloc must be able to destroy Ty");

    using ref_counter_t =
        mm::details::ref_counter_with_deleter<Ty, allocator_type, mm::details::DestroyObjectWithAllocator, mm::details::DestroyItselfAndDeallocateAll>;
    using placeholder_type         = typename shared_ptr<Ty>::template placeholder_type<ref_counter_t>;
    using placeholder_storage_type = aligned_storage_t<sizeof(placeholder_type), alignof(placeholder_type)>;

    constexpr size_t bytes_count{sizeof(placeholder_storage_type)};
    unique_ptr       placeholder_guard{
        reinterpret_cast<placeholder_storage_type*>(allocator_traits_type::allocate_bytes(alloc, bytes_count)),
    ...
    const size_t value_offset{offsetof(placeholder_type, second)};
    auto*        raw_value_ptr{reinterpret_cast<byte*>(placeholder_guard.get()) + value_offset};
    auto*        value_ptr{reinterpret_cast<Ty*>(raw_value_ptr)};
    auto*        ref_counter_ptr{reinterpret_cast<ref_counter_t*>(placeholder_guard.get())};
    construct_at(ref_counter_ptr, value_ptr, forward<Alloc>(alloc));
    allocator_traits_type::construct(ref_counter_ptr->get_deleter(), value_ptr, forward<Types>(args)...);
    placeholder_guard.release();
...
void swap(unique_ptr<Ty, Deleter>& lhs, unique_ptr<Ty, Deleter>& rhs) noexcept(noexcept(lhs.swap(rhs)))
{
    lhs.swap(rhs);
}

template <class Ty> void swap(shared_ptr<Ty>& lhs, shared_ptr<Ty>& rhs) noexcept(noexcept(lhs.swap(rhs)))
{
    lhs.swap(rhs);
}

template <class Ty> void swap(weak_ptr<Ty>& lhs, weak_ptr<Ty>& rhs) noexcept(noexcept(lhs.swap(rhs)))
{
    lhs.swap(rhs);
}

template <class Ty> Ty* get_pointer(const weak_ptr<Ty>& ptr) noexcept
{
    return ptr.get();
}
...
namespace mm::details
{
template <class Ty, class = void> struct is_reference_countable : false_type
{
};
...
struct is_reference_countable<
    Ty,
    void_t<
        decltype(intrusive_ptr_add_ref(declval<add_pointer_t<Ty>>())),
        decltype(intrusive_ptr_release(declval<add_pointer_t<Ty>>()))>> : true_type
{
};

template <class Ty> inline constexpr bool is_reference_countable_v = is_reference_countable<Ty>::value;
} // namespace mm::details

template <class Ty> class intrusive_ptr
{
  public:
    using element_type = Ty;
  ...
  public:
    ...
    constexpr intrusive_ptr(intrusive_ptr&& other) noexcept : m_ptr{other.detach()}
    {
    }

    template <class U, enable_if_t<is_convertible_v<U*, Ty*>, int> = 0>
    constexpr intrusive_ptr(const intrusive_ptr<U>& other) : m_ptr{static_cast<Ty*>(other.get())}
    {
        if (auto* ptr = get(); ptr)
        {
            intrusive_ptr_add_ref(ptr);
    ...
    template <class U, enable_if_t<is_convertible_v<U*, Ty*>, int> = 0>
    constexpr intrusive_ptr(intrusive_ptr<U>&& other) noexcept : m_ptr{static_cast<Ty*>(other.detach())}
    {
    }
    ...
    template <class U> constexpr intrusive_ptr& operator=(const intrusive_ptr<U>& other)
    {
        intrusive_ptr{other}.swap(*this);
        return *this;
    }

    template <class U> constexpr intrusive_ptr& operator=(intrusive_ptr<U>&& other) noexcept
    {
        if (this != addressof(other))
        {
    ...
    constexpr void reset(Ty* ptr = nullptr)
    {
...
constexpr bool operator==(const intrusive_ptr<Ty>& lhs, const intrusive_ptr<U>& rhs) noexcept
{
    using common_ptr_t = common_type_t<const Ty*, const U*>;
    return equal_to<common_ptr_t>{}(static_cast<common_ptr_t>(lhs.get()), static_cast<common_ptr_t>(rhs.get()));
}
...
template <class Ty> constexpr bool operator==(const intrusive_ptr<Ty>& lhs, Ty* rhs) noexcept
{
...
template <class Ty> constexpr bool operator==(Ty* lhs, const intrusive_ptr<Ty>& rhs) noexcept
{
    return rhs == lhs;
}

template <class Ty> constexpr bool operator!=(Ty* lhs, intrusive_ptr<Ty> const& rhs) noexcept
{
    return rhs != lhs;
}
...
{
    using common_ptr_t = common_type_t<const Ty*, const U*>;
    return less<common_ptr_t>{}(static_cast<common_ptr_t>(lhs.get()), static_cast<common_ptr_t>(rhs.get()));
}
...
template <class Ty> constexpr void swap(intrusive_ptr<Ty>& lhs, intrusive_ptr<Ty>& rhs) noexcept
{
    lhs.swap(rhs);
}

template <class Ty> constexpr Ty* get_pointer(const intrusive_ptr<Ty>& ptr) noexcept
{
    return ptr.get();
}
...
namespace mm::details
{
template <class Alloc, typename SizeTy> struct alloc_temporary_guard_delete
{
    using allocator_type        = Alloc;
    using allocator_traits_type = allocator_traits<allocator_type>;
    ...
    alloc_temporary_guard_delete(Alloc& alc, SizeTy cnt) : alloc{addressof(alc)}, count{cnt}
    {
    }
...
static constexpr auto make_alloc_temporary_guard(Ty* ptr, Alloc& alc, SizeTy count)
{
    ...
    return unique_ptr<typename deleter_type::value_type, deleter_type>{
        static_cast<typename deleter_type::pointer>(ptr), deleter_type{alc, count}
    };
}
} // namespace ktl
