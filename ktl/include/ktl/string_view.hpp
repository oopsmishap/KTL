...
namespace ktl
{
template <typename CharT, class Traits> class basic_winnt_string_view
{
  public:
  ...
  public:
    ...
    constexpr basic_winnt_string_view(const basic_winnt_string_view& other) noexcept = default;

    constexpr basic_winnt_string_view& operator=(const basic_winnt_string_view& other) noexcept = default;

    template <size_t N>
    constexpr basic_winnt_string_view(const value_type (&null_terminated_str)[N + 1]) noexcept
        : m_str{make_native_str(null_terminated_str, static_cast<size_type>(N))}
    {
    }

    constexpr basic_winnt_string_view(const value_type* null_terminated_str) noexcept
        : m_str{make_native_str(
              null_terminated_str, static_cast<size_type>(traits_type::length(null_terminated_str))
          )}
    {
    }

    constexpr basic_winnt_string_view(const value_type* str, size_type length) noexcept
        : m_str{make_native_str(str, length)}
    {
    }

    constexpr basic_winnt_string_view(native_string_type native_str) : m_str{native_str}
    {
    }
  ...
  public:
    ...
    constexpr basic_winnt_string_view substr(size_type pos, size_type count = npos) const noexcept
    {
    ...
    constexpr int compare(size_type pos, size_type count, basic_winnt_string_view other) const
    {
        return substr(pos, count).compare(other);
    }

    constexpr int compare(
        size_type pos, size_type count, basic_winnt_string_view other, size_type other_pos, size_type other_count
    ) const
    {
    ...
    constexpr int compare(size_type pos, size_type count, const value_type* other) const
    {
        return substr(pos, count).compare(other);
    }

    constexpr int
    compare(size_type pos, size_type count, const value_type* other, size_type other_pos, size_type other_count) const
    {
        return substr(pos, count).compare(basic_winnt_string_view(other + other_pos, other_count));
    }
    ...
    constexpr bool ends_with(basic_winnt_string_view other) const noexcept
    {
        ...
        return substr_unchecked(current_size - other_size, other_size).compare(other);
    };
    ...
    constexpr size_type find(basic_winnt_string_view other, size_type my_pos = 0) const noexcept
    {
    ...
    constexpr size_type find(const value_type* null_terminated_str, size_type my_pos = 0) const noexcept
    {
    ...
    constexpr size_type find(const value_type* str, size_t my_pos, size_type other_count) const noexcept
    {
    ...
    constexpr size_type find_first_of(basic_winnt_string_view other, size_type pos = 0) const noexcept
    {
    ...
    constexpr size_type
    find_first_of(const value_type* null_terminated_str, size_type my_pos, size_type other_count) const noexcept
    {
        return find(null_terminated_str, my_pos, other_count);
    }

    constexpr size_type find_first_of(const value_type* null_terminated_str, size_type my_pos = 0) const
    {
    ...
    size_type copy(value_type* dst, size_type count, size_type pos = 0) const
    {
        ...
        traits_type::copy(
            ...
            data() + pos,
            copied
        ); // Пользователь отвечает за то, что диапазоны не
           // должны пересекаться
        return copied;
    }

  private:
    constexpr native_string_type make_native_str(const value_type* str, size_type length) noexcept
    { // length in characters
        native_string_type native_str{};
        native_string_traits_type::set_buffer(native_str, const_cast<value_type*>(str));
        native_string_traits_type::set_size(native_str, length);
        native_string_traits_type::set_capacity(native_str, length);
        return native_str;
    }

    constexpr basic_winnt_string_view substr_unchecked(size_type pos, size_type count) const noexcept
    {
...
template <class CharT> basic_winnt_string_view(const CharT*) -> basic_winnt_string_view<CharT>;

template <
    class NativeStrTy,
    enable_if_t<cont::details::has_value_type_v<str::details::native_string_traits<NativeStrTy>>, int> = 0>
basic_winnt_string_view(NativeStrTy)
    -> basic_winnt_string_view<typename str::details::native_string_traits<NativeStrTy>::value_type>;

template <class CharT, class ChTraits>
constexpr bool
operator==(const basic_winnt_string_view<CharT, ChTraits> lhs, const basic_winnt_string_view<CharT, ChTraits> rhs) noexcept
{
...
template <
    ...
    class ChTraits,
    class Ty,
    enable_if_t<is_convertible_v<const Ty&, basic_winnt_string_view<CharT, ChTraits>>, int> = 0>
constexpr bool operator==(const basic_winnt_string_view<CharT, ChTraits> lhs, const Ty& rhs) noexcept
{
...
template <
    ...
    class ChTraits,
    enable_if_t<is_convertible_v<const Ty&, basic_winnt_string_view<CharT, ChTraits>>, int> = 0>
constexpr bool operator==(const Ty& lhs, const basic_winnt_string_view<CharT, ChTraits> rhs) noexcept
{
...
template <class CharT, class ChTraits>
constexpr bool
operator!=(basic_winnt_string_view<CharT, ChTraits> lhs, basic_winnt_string_view<CharT, ChTraits> rhs) noexcept
{
    return lhs.compare(rhs) != 0;
}
...
{
constexpr ansi_string_view operator""_asv(const char* str, size_t length) noexcept
{
    return {str, static_cast<ansi_string_view::size_type>(length)};
}

constexpr unicode_string_view operator""_usv(const wchar_t* str, size_t length) noexcept
{
