...
{
template <class Ty, align_val_t Align, template <typename, align_val_t> class BasicNodeAllocator>
class node_allocator
{
  ...
  private:
    static constexpr auto NODE_ALIGNMENT{static_cast<align_val_t>((max)(crt::CACHE_LINE_SIZE,
                                                                        static_cast<size_type>(Align)))};
  ...
  private:
     ...
     */
    using memory_block = aligned_storage_t<(max)(sizeof(memory_block_header), sizeof(Ty)), 1>;
  ...
  public:
    ...
    node_allocator(const allocator_type& alloc) noexcept(is_nothrow_copy_constructible_v<allocator_type>)
        : m_freelist{one_then_variadic_args{}, alloc}
    {
    }

    node_allocator(allocator_type&& alloc) noexcept(is_nothrow_move_constructible_v<allocator_type>)
    ...
    Ty* allocate()
    {
        ...
        while (!ptr)
        {
            ...
            {
                memory_block_header* new_top_ptr = node_pointer{old_top->next}.get_pointer();
  ...
  private:
    ...
    {
        return reinterpret_cast<Ty*>(allocator_traits_type::allocate(get_alloc(), 1));
    }

    void destroy_memory_block(Ty* target) noexcept
    {
        allocator_traits_type::deallocate(get_alloc(), reinterpret_cast<memory_block*>(target), 1);
    }
    ...
    void push_unsafe(Ty* ptr)
    {
        ...
        new_top->next.store<memory_order_relaxed>(node_pointer{current_head.get_pointer()}.get_value());
        head.store<memory_order_relaxed>(new_top.get_value());
    }

    allocator_type& get_alloc() noexcept
    {
        return m_freelist.get_first();
    }
  ...
  private:
    compressed_pair<allocator_type, memory_block_header> m_freelist{}; // aligned tagged pointer
};
} // namespace ktl::lockfree