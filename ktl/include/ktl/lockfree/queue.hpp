...
class mpmc_queue : public non_relocatable
{ // multi-producer, multi-consumer
  ...
  private:
    static constexpr auto NODE_ALIGNMENT{static_cast<align_val_t>((max)(crt::CACHE_LINE_SIZE, alignof(Ty)))};
  ...
  private:
    ...
    struct node
    {
        ...
        node(const Ty& value_) noexcept : value(value_)
        {
            ...
            next.store<memory_order_release>(node_pointer(nullptr, node_pointer{next}.get_next_tag()).get_value());
        }
    ...
    {
        node_pointer_holder& get_ptr() noexcept
        {
            return ptr;
        }
        const node_pointer_holder& get_ptr() const noexcept
        {
            return ptr;
        }
    ...
    using internal_allocator_type =
        node_allocator<node, static_cast<align_val_t>(NODE_ALIGNMENT), BasicNodeAllocator>;
  ...
  public:
    ...
    mpmc_queue(const allocator_type& alloc) : m_alc(alloc)
    {
        initialize();
    }
    mpmc_queue(allocator_type&& alloc) : m_alc(move(alloc))
    {
        initialize();
    }
    ...
    template <typename OtherTy, enable_if_t<is_constructible_v<Ty, OtherTy>, int> = 0>
    bool unsynchronized_push(const OtherTy& value)
    {
        return unsynchronized_push_impl(value);
    }

    template <typename OtherTy, enable_if_t<is_convertible_v<OtherTy, Ty> && is_nothrow_assignable_v<OtherTy&, Ty>, int> = 0>
  ...
  private:
    void initialize()
    {
        static_assert(is_trivially_destructible_v<Ty>, "Ty must be trivially destructible");
    ...
    void destroy_node(node_pointer target)
    {
        ...
        allocator_traits_type::deallocate_single_object(m_alc, target.get_pointer());
    }

    template <typename OtherTy> bool unsynchronized_push_impl(const OtherTy& value)
    {
        auto new_node{create_data_node(value)};
        ...
        for (;;)
        {
            ...
            if (!next)
            {
                tail->next.store<memory_order_relaxed>(node_pointer{new_node, next.get_next_tag()}.get_value());
    ...
    template <bool CopyToOutput, typename OtherTy> bool unsynchronized_pop_impl(OtherTy& value)
    {
        for (;;)
        {
            ...
            if (auto tail = node_pointer{m_tail.get_ptr().load<memory_order_relaxed>()}; head == tail)
            {
                if (!next)
                {
                    return false;
    ...
    static bool cas_strong_helper(node_pointer_holder& place, node_pointer expected, node_pointer new_ptr) noexcept
    {
        auto expected_value{expected.get_value()};
        return place.compare_exchange_strong(expected_value, new_ptr.get_value());
    }

    static bool cas_weak_helper(node_pointer_holder& place, node_pointer expected, node_pointer new_ptr) noexcept
    {
        auto expected_value{expected.get_value()};
...
template <class Ty> using queue = mpmc_queue<Ty, aligned_paged_allocator>;

template <class Ty> using queue_non_paged = mpmc_queue<Ty, aligned_non_paged_allocator>;
} // namespace ktl::lockfree
