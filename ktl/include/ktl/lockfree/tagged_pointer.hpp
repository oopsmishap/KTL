...
{
template <class Ty> class tagged_pointer
{
  public:
    using value_type = Ty;
    using reference  = Ty&;
  ...
  private:
    ...
    static constexpr placeholder_type KERNEL_MODE_ADDRESS_MASK{0xFFFF'0000'0000'0000ull};
  ...
  public:
    ...
    explicit tagged_pointer(placeholder_type number) noexcept : m_ptr{from_number(number)}
    {
    }

    explicit tagged_pointer(pointer ptr, tag_type tag = 0) noexcept : m_ptr{pack_ptr(ptr, tag)}
    {
    }
    ...
    pointer get_pointer() const volatile noexcept
    {
        return extract_ptr(m_ptr);
    }

    placeholder_type get_value() const noexcept
    {
        return to_number(m_ptr);
    }
    ...
    {
        return m_ptr.address != 0;
    }

  private:
    static constexpr Ty* extract_ptr(const volatile compressed_pointer& ptr) noexcept
    {
        return reinterpret_cast<Ty*>(ptr.address | KERNEL_MODE_ADDRESS_MASK);
    }

    static constexpr tag_type extract_tag(const volatile compressed_pointer& ptr) noexcept
    {
        return static_cast<tag_type>(ptr.tag);
    }

    static constexpr compressed_pointer pack_ptr(Ty* addr, tag_type tag) noexcept
    {
    ...
    static constexpr compressed_pointer from_number(placeholder_type number) noexcept
    {
        compressed_pointer ptr;
...
template <class Ty> bool operator==(const tagged_pointer<Ty>& lhs, const tagged_pointer<Ty>& rhs) noexcept
{
