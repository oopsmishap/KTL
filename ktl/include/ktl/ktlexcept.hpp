...
struct winnt_exception_base : exception
{
    ...
    winnt_exception_base(const basic_ansi_string_view<Traits>& str) : MyBase(data(str), size(str))
    {
    }

    template <class Traits = char_traits<wchar_t>>
    winnt_exception_base(const basic_unicode_string_view<Traits>& str) : MyBase(data(str), size(str))
    {
    }

    template <class Traits = char_traits<char>>
    constexpr winnt_exception_base(persistent_message_t, const basic_ansi_string_view<Traits>& str)
        : MyBase(persistent_message_t{}, data(str))
    {
    }

    template <size_t N> constexpr winnt_exception_base(const char (&str)[N]) : MyBase(str)
    {
    }
};
} // namespace exc::details
...
{
    using MyBase = logic_error;
    constexpr future_error() noexcept : MyBase{"future has no associated shared state"}
    {
    }
    [[nodiscard]] NTSTATUS code() const noexcept override;
};
...
struct kernel_error : runtime_error
{
    ...
    template <class Traits = char_traits<char>>
    kernel_error(NTSTATUS code, const basic_ansi_string_view<Traits>& str) : MyBase(str)
                                                                           , m_code{code}
    {
    }

    template <class Traits = char_traits<wchar_t>>
    kernel_error(NTSTATUS code, const basic_unicode_string_view<Traits>& str) : MyBase(str)
                                                                              , m_code{code}
    {
    }

    template <class Traits = char_traits<char>>
    constexpr kernel_error(persistent_message_t, NTSTATUS code, const basic_ansi_string_view<Traits>& str)
        : MyBase(persistent_message_t{}, str)
        , m_code{code}
    {
    }

    template <size_t N>
    constexpr kernel_error(NTSTATUS code, const char (&str)[N]) : MyBase(str)
                                                                , m_code{code}
    {
    }
...
// slow path
template <class Exc, class... Types> [[noreturn]] NOINLINE void throw_exception(Types&&... args)
{
