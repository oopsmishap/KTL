...
namespace ktl
{
template <typename CharT, size_t SsoBufferChCount, class Traits, class Alloc> class basic_winnt_string
{
  public:
    using native_string_traits_type = str::details::native_string_traits_selector<CharT>;
  ...
  private:
    template <class Ty> struct not_convertible_to_native_str
    {
        static constexpr bool value = !is_convertible_v<const Ty&, const_pointer>
  ...
  public:
    constexpr basic_winnt_string() noexcept
    {
        become_small();
    }

    explicit constexpr basic_winnt_string(const allocator_type& alloc
    ...
    explicit constexpr basic_winnt_string(allocator_type&& alloc
    ...
    template <class Allocator = allocator_type, enable_if_t<is_constructible_v<allocator_type, Allocator>, int> = 0>
    basic_winnt_string(size_type count, value_type ch, Allocator&& alloc = Allocator{})
    ...
    basic_winnt_string(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& other, size_type pos)
        : basic_winnt_string(other, pos, other.size() - pos)
    { // It is ok to underflow the unsigned number
    }

    template <size_t BufferSize, class ChTraits, class ChAlloc>
    basic_winnt_string(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& other, size_type pos, size_type count)
        : m_str{
              one_then_variadic_args{},
              allocator_traits_type::select_on_container_copy_construction(other.get_alloc())
          }
    {
        become_small();
    ...
    template <
        ...
        enable_if_t<is_convertible_v<Ty, string_view_type> && not_convertible_to_native_str<Ty>::value, int> = 0>
    basic_winnt_string(const Ty& value, Allocator&& alloc = Allocator{})
        : basic_winnt_string(*static_cast<string_view_type>(value).raw_str(), forward<Allocator>(alloc))
    {
    }

    template <class Allocator = allocator_type, enable_if_t<is_constructible_v<allocator_type, Allocator>, int> = 0>
    basic_winnt_string(const value_type* str, size_type length, Allocator&& alloc = Allocator{})
        : m_str{one_then_variadic_args{}, forward<Allocator>(alloc)}
    {
        become_small();
    ...
    template <class Allocator = allocator_type, enable_if_t<is_constructible_v<allocator_type, Allocator>, int> = 0>
    basic_winnt_string(const value_type* null_terminated_str, Allocator&& alloc = Allocator{})
        : m_str{one_then_variadic_args{}, forward<Allocator>(alloc)}
    {
        become_small();
        concat_without_transfer_old_data(
            make_copy_helper(), static_cast<size_type>(traits_type::length(null_terminated_str)), null_terminated_str
        );
    }

    template <class InputIt, class Allocator = allocator_type, enable_if_t<is_constructible_v<allocator_type, Allocator>, int> = 0>
    basic_winnt_string(InputIt first, InputIt last, Allocator&& alloc = Allocator{})
        : m_str{one_then_variadic_args{}, forward<Allocator>(alloc)}
    {
        become_small();
        append_range_impl<false_type>(
            first, last, is_same<typename iterator_traits<InputIt>::iterator_category, random_access_iterator_tag>{}
        );
    }

    template <class Allocator = allocator_type, enable_if_t<is_constructible_v<allocator_type, Allocator>, int> = 0>
    basic_winnt_string(native_string_type native_str, Allocator&& alloc = Allocator{})
        : m_str{one_then_variadic_args{}, forward<Allocator>(alloc)}
    {
        become_small();
        concat_without_transfer_old_data(
            make_copy_helper(), native_string_traits_type::get_size(native_str), native_str.Buffer
        );
    }

    template <size_t BufferSize, class ChTraits, class ChAlloc, enable_if_t<BufferSize != SSO_BUFFER_CH_COUNT, int> = 0>
    basic_winnt_string(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& other)
        : basic_winnt_string(
              *other.raw_str(), allocator_traits_type::select_on_container_copy_construction(other.get_alloc())
          )
    {
    }
    ...
    basic_winnt_string(basic_winnt_string&& other) noexcept(is_nothrow_move_constructible_v<allocator_type>)
    ...
    template <size_t BufferSize, class ChTraits, class ChAlloc>
    basic_winnt_string& assign(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& other)
    {
    ...
    template <size_t BufferSize, class ChTraits, class ChAlloc>
    basic_winnt_string&
    assign(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& other, size_type pos, size_type count = npos)
    {
        clear();
        append_by_pos_and_count<false>(other, pos, count);
        return *this;
    ...
    {
        return assign(null_terminated_str, static_cast<size_type>(traits_type::length(null_terminated_str)));
    }

    template <class InputIt> basic_winnt_string& assign(InputIt first, InputIt last)
    {
        clear();
        append_range_impl<false_type>(
            first, last, is_same<typename iterator_traits<InputIt>::iterator_category, random_access_iterator_tag>{}
    ...
    template <size_t BufferSize, class ChTraits, class ChAlloc, enable_if_t<BufferSize != SSO_BUFFER_CH_COUNT, int> = 0>
    basic_winnt_string& operator=(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& other)
    {
    ...
    basic_winnt_string& operator=(basic_winnt_string&& other
    ) noexcept(is_same_v<typename allocator_traits_type::is_always_equal, true_type> || is_same_v<typename allocator_traits_type::propagate_on_container_move_assignment, true_type> && is_nothrow_move_assignable_v<allocator_type>)
    {
        ...
        if (this != addressof(other))
        {
            move_assignment_impl<move_alloc>(
                move(other), typename allocator_traits_type::propagate_on_container_move_assignment{}
            );
        }
        return *this;
    }
    ...
    void shrink_to_fit()
    {
        if (!is_small())
        {
            ...
            if (current_capacity > current_size)
            {
                ...
                native_string_traits_type::set_buffer(get_native_str(), m_str, new_buffer);
                native_string_traits_type::set_capacity(get_native_str(), m_str, current_size);
    ...
    basic_winnt_string& append(const basic_winnt_string<CharT, BufferSize, Traits, ChAlloc>& str)
    {
    ...
    {
        return append(native_str.Buffer, native_string_traits_type::get_size(native_str));
    }
    ...
    template <size_t BufferSize, class ChAlloc>
    basic_winnt_string&
    append(const basic_winnt_string<CharT, BufferSize, Traits, ChAlloc>& str, size_type pos, size_type count)
    {
    ...
    {
        return append(null_terminated_str, static_cast<size_type>(traits_type::length(null_terminated_str)));
    }

    template <class InputIt> basic_winnt_string& append(InputIt first, InputIt last)
    {
    ...
    // traits_type must be equivalent
    template <size_t BufferSize, class ChAlloc>
    basic_winnt_string& operator+=(const basic_winnt_string<CharT, BufferSize, Traits, ChAlloc>& str)
    {
        return append(str);
    }

    basic_winnt_string& operator+=(native_string_type str)
    {
        return append(str);
    }
    ...
    template <size_t BufferSize, class ChAlloc>
    [[nodiscard]] constexpr int compare(const basic_winnt_string<CharT, BufferSize, Traits, ChAlloc>& other
    ) const noexcept
    {
        return compare_unchecked(data(), size(), other.data(), other.size());
    }

    [[nodiscard]] constexpr int compare(size_type my_pos, size_type my_count, native_string_type native_str) const
    {
        ...
        return native_string_traits_type::compare_unchecked(
            data() + my_pos,
            (min)(my_count, current_size - my_pos),
    ...
    [[nodiscard]] constexpr int compare(
        size_type my_pos, size_type my_count, const basic_winnt_string<CharT, BufferSize, Traits, ChAlloc>& other
    ) const
    {
    ...
    [[nodiscard]] constexpr int compare(const value_type* null_terminated_str) const
    {
        return compare_unchecked(
            data(), size(), null_terminated_str, static_cast<size_type>(traits_type::length(null_terminated_str))
        );
    }

    [[nodiscard]] constexpr int
    compare(size_type my_pos, size_type my_count, const value_type* null_terminated_str) const
    {
        return compare(
            my_pos, my_count, null_terminated_str, static_cast<size_type>(traits_type::length(null_terminated_str))
        );
    }

    [[nodiscard]] constexpr int
    compare(size_type my_pos, size_type my_count, const value_type* str, size_type other_count) const
    {
        const size_type current_size{size()};
        cont::details::throw_if_index_greater_than_size(my_pos, current_size);
        const value_type* substr{data() + my_pos};
        const auto        substr_length{(min)(static_cast<size_type>(current_size - my_pos), my_count)};
        return compare_unchecked(substr, substr_length, str, other_count);
    }

    [[nodiscard]] constexpr int compare(native_string_type native_str) const noexcept
    {
        return compare_unchecked(
            data(),
            size(),
    ...
    [[nodiscard]] constexpr int
    compare(native_string_type native_str, size_type other_pos, size_type other_count) const noexcept
    {
    ...
    template <
        ...
        class Allocator = allocator_type,
        enable_if_t<is_convertible_v<Ty, string_view_type> && not_convertible_to_native_str<Ty>::value, int> = 0>
    int compare(const Ty& value) noexcept(is_nothrow_convertible_v<Ty, string_view_type>)
    {
    ...
    template <
        ...
        enable_if_t<is_convertible_v<Ty, string_view_type> && not_convertible_to_native_str<Ty>::value, int> = 0>
    int compare(
        size_type my_pos, size_type my_count, const Ty& value
    ) noexcept(is_nothrow_convertible_v<Ty, string_view_type>)
    {
        return compare(my_pos, my_count, *static_cast<string_view_type>(value).raw_str());
    }
    ...
    template <
        ...
        enable_if_t<is_convertible_v<Ty, string_view_type> && not_convertible_to_native_str<Ty>::value, int> = 0>
    int compare(
        size_type my_pos, size_type my_count, const Ty& value, size_type other_pos, size_type other_count
    ) noexcept(is_nothrow_convertible_v<Ty, string_view_type>)
    {
        return compare(my_pos, my_count, *static_cast<string_view_type>(value).raw_str(), other_pos, other_count);
    }
    ...
    basic_winnt_string& insert(size_type index, const value_type* str, size_type count)
    {
        const size_type current_size{size()};
    ...
    basic_winnt_string& insert(size_type index, const value_type* null_terminated_str)
    {
        return insert(index, null_terminated_str, static_cast<size_type>(traits_type::length(null_terminated_str)));
    }

    basic_winnt_string& insert(size_type index, native_string_type native_str)
    {
        return insert(
            index, native_string_traits_type::get_buffer(native_str), native_string_traits_type::get_size(native_str)
        );
    }

    template <size_t BufferSize, class ChAlloc>
    basic_winnt_string& insert(size_type index, const basic_winnt_string<CharT, BufferSize, Traits, ChAlloc>& other)
    {
        return insert(index, *other.raw_str());
    }

    template <size_t BufferSize, class ChAlloc>
    basic_winnt_string& insert(size_type index, const basic_winnt_string_view<CharT, Traits> other)
    {
        return insert(index, *other.raw_str());
    }
    ...
    )
    {
        ...
        cont::details::throw_if_index_greater_than_size(other_pos, other_size);
        insert_impl(index, make_copy_helper(), (min)(other_size - other_pos, other_count), other.data() + other_pos);
        return *this;
    }
    ...
    template <class InputIt> iterator insert(const_iterator pos, InputIt first, InputIt last)
    {
        ...
        const auto count{insert_range_impl(
            ...
            first,
            last,
            is_same<typename iterator_traits<InputIt>::iterator_category, random_access_iterator_tag>{}
        )};
        return begin() + index + count;
    }
    ...
    template <
        ...
        class Allocator = allocator_type,
        enable_if_t<is_convertible_v<Ty, string_view_type> && not_convertible_to_native_str<Ty>::value, int> = 0>
    basic_winnt_string& insert(size_type index, const Ty& value, size_type sv_pos, size_type sv_count)
    {
    ...
    basic_winnt_string& erase(size_type index = 0, size_type count = npos)
    {
        ...
        erase_unchecked(index, (min)(count, static_cast<size_type>(old_size - index)));
        return *this;
    }
    ...
    constexpr bool starts_with(const value_type* null_terminated_str) const noexcept
    {
        return static_cast<string_view_type>(*this).starts_with(null_terminated_str);
    }
    ...
    constexpr bool ends_with(const value_type* null_terminated_str) const noexcept
    {
    ...
    constexpr bool contains(const value_type* null_terminated_str) const noexcept
    {
        return find(null_terminated_str) != npos;
    }

    [[nodiscard]] basic_winnt_string substr(size_type pos = 0, size_type count = npos)
    {
        size_type current_size{size()};
        cont::details::throw_if_index_greater_than_size(pos, current_size);
        return basic_winnt_string{data() + pos, (min)(static_cast<size_type>(current_size - pos), count)};
    }
    ...
    void swap(basic_winnt_string& other) noexcept
    {
        ktl::swap(*this, other);
    }
    ...
    template <size_t BufferSize, class ChAlloc>
    constexpr size_type
    find(const basic_winnt_string<CharT, BufferSize, Traits, ChAlloc>& other, size_type my_pos = 0) const noexcept
    {
        return str::details::find_substr<traits_type>(data(), my_pos, size(), other.data(), other.size(), npos);
    }

    constexpr size_type find(const value_type* null_terminated_str, size_type my_pos, size_type str_count) const noexcept
    {
        return static_cast<string_view_type>(*this).find(null_terminated_str, my_pos, str_count);
    }

    constexpr size_type find(const value_type* null_terminated_str) const noexcept
    {
    ...
    template <size_t BufferSize, class ChAlloc>
    constexpr size_type
    find_first_of(const basic_winnt_string<CharT, BufferSize, Traits, ChAlloc>& other, size_type my_pos) const noexcept
    {
        return find(other, my_pos);
    }

    constexpr size_type
    find_first_of(const value_type* null_terminated_str, size_type str_pos, size_type str_count) const noexcept
    {
        return find(null_terminated_str, str_pos, str_count);
    }

    constexpr size_type find_first_of(const value_type* null_terminated_str) const noexcept
    {
  ...
  private:
    constexpr void become_small(size_type size = 0) noexcept
    {
        ...
        native_string_traits_type::set_capacity(get_native_str(), SSO_BUFFER_CH_COUNT);
    }
    ...
    constexpr pointer get_sso_buffer() noexcept
    {
        return ktl::data(m_buffer);
    }

    template <bool CalcOptimalGrowth, size_t BufferSize, class ChTraits, class ChAlloc>
    void append_by_pos_and_count(
        const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& other, size_type pos, size_type count
    )
    {
        ...
        {
            concat_with_optimal_growth(make_copy_helper(), (min)(length, count), other.data() + pos);
        }
        else
        {
            concat_without_transfer_old_data(make_copy_helper(), (min)(length, count), other.data() + pos);
        }
    }
    ...
    void append_range_impl(RandomAccessIt first, RandomAccessIt last, true_type)
    {
        ...
        append_range_by_random_access_iterators(first, last, range_length, OptimalGrowthTag{});
    }

    template <class RandomAccessIt>
    void append_range_by_random_access_iterators(
        RandomAccessIt first, [[maybe_unused]] RandomAccessIt last, size_type range_length, false_type
    )
    {
    ...
    size_type insert_range_impl(size_type index, InputIt first, InputIt last, false_type)
    {
    ...
    template <class RandomAccessIt>
    size_type insert_range_impl(size_type index, RandomAccessIt first, RandomAccessIt last, true_type)
    {
        const auto range_length{static_cast<size_type>(distance(first, last))};
    ...
    void insert_range_by_random_access_iterators(
        size_type index, RandomAccessIt first, [[maybe_unused]] RandomAccessIt last, size_type range_length
    )
    {
        if constexpr (is_pointer_v<RandomAccessIt>)
        {
    ...
    template <class RandomAccessIt>
    void append_range_by_random_access_iterators(
        RandomAccessIt first, [[maybe_unused]] RandomAccessIt last, size_type range_length, true_type
    )
    {
        if constexpr (is_pointer_v<RandomAccessIt>)
        {
        ...
        {
            concat_with_optimal_growth(make_copy_range_helper(), range_length, first, last);
        }
    }

    template <size_t BufferSize, class ChTraits, class ChAlloc>
    void copy_assignment_impl(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& other, true_type)
    {
        destroy();
        become_small();
        get_alloc() = other.get_alloc();
        concat_without_transfer_old_data(make_copy_helper(), other.size(), other.data());
    }

    template <size_t BufferSize, class ChTraits, class ChAlloc>
    void copy_assignment_impl(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& other, false_type)
    {
        clear();
        concat_without_transfer_old_data(make_copy_helper(), other.size(), other.data());
    }

    template <bool AllocIsAlwaysEqual> void move_assignment_impl(basic_winnt_string&& other, false_type)
    {
        ...
        {
            copy_assignment_impl(static_cast<const basic_winnt_string&>(other), false_type{});
            other.destroy();
            other.become_small();
        }
    }

    template <bool MoveAlloc> void move_assignment_impl(basic_winnt_string&& other, true_type)
    {
        destroy();
        if (!other.is_small())
        {
    ...
    void concat_with_optimal_growth(Handler handler, size_type count, const Types&... args)
    {
        concat_impl(
            handler,
    ...
    template <class Handler, typename... Types>
    void concat_without_transfer_old_data(Handler handler, size_type count, const Types&... args)
    {
        concat_impl(
            handler,
            [this](size_type required)
    ...
    void concat_impl(Handler handler, Reallocator reallocator, size_type count, const Types&... args)
    {
    ...
    template <class Handler, typename... Types>
    void insert_impl(size_type index, Handler handler, size_type count, const Types&... args)
    {
        const auto old_size{size()}, required{static_cast<size_type>(old_size + count)},
            need_to_shift{static_cast<size_type>(old_size - index)};
        if (value_type* base = data() + index; required > capacity())
        {
            grow(calc_optimal_growth(required), make_shifter(make_copy_helper(), {index, count}));
        }
        else
        {
    ...
    template <class TransferPolicy> void grow(size_type new_capacity, TransferPolicy transfer_handler)
    {
        throw_exception_if_not<length_error>(new_capacity <= max_size(), "string is too long");
    ...
    void grow_unchecked(size_type new_capacity, TransferPolicy transfer_handler)
    {
        ...
        value_type*     new_buffer{
            new_capacity <= SSO_BUFFER_CH_COUNT ? get_sso_buffer() : allocate_buffer(alc, new_capacity)
        };
        const size_type new_size{transfer_handler(new_buffer, old_size, old_buffer)};
        if (!is_small())
        {
    ...
    static void shift_right_helper(
        Handler handler, value_type* dst, const value_type* src, size_type count, pair<size_type, size_type> bounds
    ) noexcept(noexcept(handler(dst, count, src)))
    {
    ...
    template <class Handler>
    static constexpr auto make_shifter(Handler handler, pair<size_type, size_type> bounds)
    {
        return [handler, bounds](value_type* dst, size_type count, const value_type* src) noexcept(
                   noexcept(handler(dst, count, src))
               ) -> size_type
        {
    ...
    {
        return []([[maybe_unused]] value_type* dst, size_type count, [[maybe_unused]] const value_type* src) noexcept
        {
            return count;
        };
    }
    ...
    {
        return [](value_type* dst, size_type count, const value_type* src) noexcept
        {
            traits_type::copy(dst, src, count);
        };
    }

    template <class InputIt> static constexpr auto make_copy_range_helper() noexcept
    {
    ...
    static void deallocate_buffer(allocator_type& alc, value_type* buffer, size_type ch_count) noexcept
    {
        return allocator_traits_type::deallocate(alc, buffer, ch_count);
    }

    static constexpr size_type calc_growth_unchecked(size_type old_capacity) noexcept
    {
        return old_capacity * GROWTH_MULTIPLIER;
    }

    static constexpr int
    compare_unchecked(const value_type* lhs, size_type lhs_count, const value_type* rhs, size_type rhs_count) noexcept
    {
...
template <class CharT> basic_winnt_string(const CharT*) -> basic_winnt_string<CharT>;

template <
    class NativeStrTy,
    enable_if_t<cont::details::has_value_type_v<str::details::native_string_traits<NativeStrTy>>, int> = 0>
basic_winnt_string(NativeStrTy)
    -> basic_winnt_string<typename str::details::native_string_traits<NativeStrTy>::value_type>;

template <typename CharT, size_t LhsBufferSize, size_t RhsBufferSize, class ChTraits, class LhsChAlloc, class RhsChAlloc>
auto operator+(
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
auto operator+(const basic_winnt_string<typename CharT, BufferSize, ChTraits, ChAlloc>& lhs, const CharT* null_terminated_str)
{
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
auto operator+(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& lhs, CharT ch)
{
    basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc> str{lhs};
    str.push_back(ch);
    return str;
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
auto operator+(basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>&& lhs, const CharT* null_terminated_str)
{
    return move(lhs += null_terminated_str);
}

template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
auto operator+(basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>&& lhs, CharT ch)
{
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
auto operator+(const CharT* null_terminated_str, const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& rhs)
{
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
auto operator+(CharT ch, const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& rhs)
{
    basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc> str{rhs};
    str.push_back(ch);
    return str;
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
auto operator+(const CharT* null_terminated_str, basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>&& rhs)
{
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
auto operator+(
    typename basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>::value_type ch,
...
template <typename CharT, size_t LhsBufferSize, size_t RhsBufferSize, class RhsChTraits, class LhsChTraits, class LhsChAlloc, class RhsChAlloc>
auto operator+(
    const basic_winnt_string<CharT, LhsBufferSize, LhsChTraits, LhsChAlloc>& lhs,
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
auto operator+(
    basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>&& lhs,
    basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>&& rhs
)
{
    ...
    const size_type lhs_size{lhs.size()}, rhs_size{rhs.size()}, lhs_capacity{lhs.capacity()},
        rhs_capacity{rhs.capacity()};
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
constexpr bool
operator==(const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& lhs, const CharT* null_terminated_str) noexcept
{
...
template <typename CharT, size_t BufferSize, class ChTraits, class ChAlloc>
constexpr bool
operator==(const CharT* null_terminated_str, const basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>& rhs) noexcept
{
...
constexpr bool operator==(
    ...
    typename basic_winnt_string<CharT, BufferSize, ChTraits, ChAlloc>::native_string_type native_str
) noexcept
{
