...
namespace ktl
{
template <class Ty, class Allocator = basic_paged_allocator<Ty>> class vector
{
  public:
    using value_type = Ty;
  ...
  public:
    static_assert(is_same_v<Ty, typename allocator_traits_type::value_type>, "Incompatible allocator");

  private:
    template <class InputIt>
    using forward_or_greater_t =
        is_base_of<typename iterator_traits<InputIt>::iterator_category, forward_iterator_tag>;

  private:
    static constexpr size_type MIN_CAPACITY{1}, GROWTH_MULTIPLIER{2};

  public:
    vector() noexcept(is_nothrow_default_constructible_v<allocator_type>) = default;

    explicit vector(const allocator_type& alloc
    ) noexcept(is_nothrow_constructible_v<allocator_type, const allocator_type&>)
        : m_impl{one_then_variadic_args{}, alloc}
    {
    }

    explicit vector(allocator_type&& alloc) noexcept(is_nothrow_constructible_v<allocator_type, allocator_type&&>)
        : m_impl{one_then_variadic_args{}, move(alloc)}
    {
    }

    template <class Alloc = allocator_type, enable_if_t<is_constructible_v<allocator_type, Alloc>, int> = 0>
    vector(Alloc&& alloc = Alloc{}) noexcept(is_nothrow_constructible_v<allocator_type, Alloc>)
    ...
    template <
        ...
        enable_if_t<is_constructible_v<allocator_type, Alloc> && is_default_constructible_v<value_type>, int> = 0>
    explicit vector(
        size_type object_count, Alloc&& alloc = Alloc{}
    ) noexcept(is_nothrow_constructible_v<allocator_type, Alloc> && is_nothrow_default_constructible_v<value_type>)
        : m_impl{one_then_variadic_args{}, forward<Alloc>(alloc)}
    {
        grow<true>(object_count, make_default_construct_helper(0), make_dummy_transfer(), object_count);
    }

    template <
        class Alloc = allocator_type,
        enable_if_t<is_constructible_v<allocator_type, Alloc> && is_copy_constructible_v<value_type>, int> = 0>
    explicit vector(
        size_type object_count, const Ty& value, Alloc&& alloc = Alloc{}
    ) noexcept(is_nothrow_constructible_v<allocator_type, Alloc> && is_nothrow_default_constructible_v<Ty>)
        : m_impl{one_then_variadic_args{}, forward<Alloc>(alloc)}
    {
        grow<true>(object_count, make_construct_fill_helper(0), make_dummy_transfer(), value, object_count);
    }
    ...
    template <
        ...
        class Alloc = allocator_type,
        enable_if_t<
            is_constructible_v<allocator_type, Alloc>
                && is_base_of_v<input_iterator_tag, typename iterator_traits<InputIt>::iterator_category>,
            int> = 0>
    ...
    {
        grow_unchecked<true>(other.size(), make_cloner(), make_dummy_transfer(), other);
    }

    template <class Alloc = allocator_type, enable_if_t<is_constructible_v<allocator_type, Alloc>, int> = 0>
    vector(const vector& other, Alloc&& alloc) : m_impl{one_then_variadic_args{}, forward<Alloc>(alloc)}
    {
        grow_unchecked<true>(other.size(), make_cloner(), make_dummy_transfer(), other);
    }

    vector(vector&& other) noexcept(is_nothrow_move_constructible_v<allocator_type>)
        : m_impl{move(other.m_impl)}
    {
    }

    template <class Alloc = allocator_type, enable_if_t<is_constructible_v<allocator_type, Alloc>, int> = 0>
    vector(vector&& other, Alloc&& alloc) : m_impl{one_then_variadic_args{}, forward<Alloc>(alloc)}
    {
    ...
    void assign(size_type count, const Ty& value)
    {
        if (count > capacity())
        {
            grow<true>(count, make_construct_fill_helper(0), make_dummy_transfer(), value, count);
        }
        else
        {
    ...
    template <
        class InputIt,
        enable_if_t<is_base_of_v<input_iterator_tag, typename iterator_traits<InputIt>::iterator_category>, int> = 0>
    ...
    void reserve(size_type new_capacity)
    {
        ...
        {
            grow<false>(new_capacity, make_dummy_construct_helper(), make_transfer_without_shift(), size());
        }
    }
    ...
    void shrink_to_fit()
    {
        ...
        {
            grow<false>(current_size, make_dummy_construct_helper(), make_transfer_without_shift(), current_size);
        }
    }
    ...
    iterator insert(const_iterator pos, size_type count, const Ty& value)
    {
        ...
        if (const size_type required = current_size + count; required > capacity())
        {
            grow<true>(
                calc_optimal_growth(required),
                make_construct_fill_helper(offset),
    ...
    {
        return insert_range(
            pos, first, last, is_base_of<forward_iterator_tag, typename iterator_traits<InputIt>::iterator_category>{}
        );
    }

    template <class... Types> iterator emplace(const_iterator pos, Types&&... args)
    {
        if (pos == cend())
        {
        ...
        if (current_size == capacity())
        {
            grow<true>(
                calc_growth(),
                make_construct_at_helper(offset),
        ...
        else
        {
            ...
            pointer buffer{data()};
            make_construct_at_helper(current_size)(get_alloc(), buffer, move_if_noexcept(back()));
            ++get_size();
    ...
    iterator erase(const_iterator first, const_iterator last)
    {
        ...
        auto my_first{begin()};
        assert_with_msg(first >= my_first && last <= end(), "iterator range does not belong to the vector");
        const auto offset{static_cast<size_type>(first - my_first)}, count{static_cast<size_type>(last - first)};
        if (first == last)
        {
    ...
    template <class... Types> reference emplace_back(Types&&... args)
    {
        const size_type current_size{size()};
        if (current_size == capacity())
        {
            grow<true>(
                calc_growth(),
                make_construct_at_helper(current_size),
                make_transfer_without_shift(),
                forward<Types>(args)...
            );
        }
        else
        {
            make_construct_at_helper(current_size)(get_alloc(), data(), forward<Types>(args)...);
            ++get_size();
        }
        return back();
    }
    ...
    void resize(size_type count)
    {
        resize_impl(
            count,
            make_default_construct_helper(size()),
            count - size()
        ); // Unsigned underflow is allowed by C++ Standard. If
    ...
    {
        resize_impl(count, make_construct_fill_helper(size()), value, count - size());
    }

    void swap(vector& other) noexcept(
        allocator_traits_type::propagate_on_container_swap::value || allocator_traits_type::is_always_equal::value
    )
    {
        if (addressof(other) != this)
        {
            swap_impl(other, typename allocator_traits_type::propagate_on_container_swap{});
        }
    }
  ...
  private:
    ...
    template <class InputIt> void construct_from_range(InputIt first, InputIt last, false_type)
    {
    ...
    template <class ForwardIt> void construct_from_range(ForwardIt first, ForwardIt last, true_type)
    {
        const auto range_length{static_cast<size_type>(distance(first, last))};
        grow_with_optional_check_and_adjust_resize<true>(
            range_length, make_range_construct_helper(0), make_dummy_transfer(), first, range_length
        );
    }
    ...
    void assign_range(InputIt first, InputIt last, false_type)
    {
        ...
        for (pointer buffer = data(); idx < current_size && first != last; first = next(first), ++idx)
        {
            *buffer++ = *first;
        }
        if (idx == current_size)
    ...
    {
        const size_type current_size{size()}, range_length{static_cast<size_type>(distance(first, last))};
        ...
        else
        {
            ...
            uninitialized_copy_n_unchecked(
                subrange_end, range_length - current_size, data() + current_size, get_alloc()
            );
            get_size() = range_length;
        }
    }
    ...
    void move_assignment_impl(vector&& other, false_type)
    {
        ...
        else
        {
            ...
            {
                grow_unchecked(other_size, make_taker(), make_transfer_without_shift(), move(other));
            }
            else
            {
    ...
    template <class InputIt>
    iterator insert_range(const_iterator pos, InputIt first, InputIt last, false_type)
    {
        size_type count{0};
    ...
    iterator insert_range(const_iterator pos, ForwardIt first, ForwardIt last, true_type)
    {
        const auto      range_length{distance(first, last)};
        const size_type current_size{size()};
        const auto      offset{static_cast<size_type>(pos - begin())};

        if (const size_type required = current_size + range_length; required > capacity())
        {
            grow<true>(
                calc_optimal_growth(required),
    ...
    void append_n_without_grow(size_type count, const Ty& value)
    {
        ...
        get_size() += make_construct_fill_helper(current_size)(get_alloc(), data(), value, count);
    }

    void insert_n_without_grow(size_type count, const Ty& value, size_type offset)
    {
        ...
        if (count <= residue)
        {
            ...
            make_transfer_without_shift()(alloc, buffer_end, count, subrange_first + unshifted);
            get_size() += count;
        ...
        else
        {
            ...
            get_size() += make_construct_fill_helper(current_size)(alloc, buffer, value, in_raw_memory);
    ...
    template <class ForwardIt> void append_range_without_grow(ForwardIt first, size_type range_length)
    {
        const size_type current_size{size()};
        assert(current_size + range_length <= capacity());
        get_size() += make_range_construct_helper(current_size)(get_alloc(), data(), first, range_length);
    }

    template <class ForwardIt>
    void insert_range_without_grow(ForwardIt first, size_type range_length, size_type offset)
    {
        const size_type current_size{size()}, residue{current_size - offset};
        assert(current_size + range_length <= capacity());
        ...
        if (range_length <= residue)
        {
            ...
            pointer         buffer_end{buffer + current_size};
            make_transfer_without_shift()(alloc, buffer_end, range_length, subrange_first + unshifted);
            get_size() += range_length;
        ...
        else
        {
            ...
            get_size() +=
                make_range_construct_helper(current_size)(alloc, buffer, first_not_copied, in_raw_memory);
            pointer buffer_end{buffer + size()};
            make_transfer_without_shift()(alloc, buffer_end, residue, subrange_first);
            get_size() += residue;
    ...
    void resize_impl(size_type count, ConstructionPolicy construction_handler, Types&&... args)
    {
        const size_type current_size{size()};
        ...
        else if (count > current_size)
        {
            ...
            {
                grow<false>(count, construction_handler, make_transfer_without_shift(), forward<Types>(args)...);
            }
        }
        get_size() = count;
    }
    ...
    void swap_impl(vector& other, false_type) noexcept(allocator_traits_type::is_always_equal::value)
    {
        if (alc::details::allocators_are_equal(get_alloc(), other.get_alloc()))
        {
            ::swap(m_impl.get_second(), other.m_impl.get_second());
        }
        assert_with_msg(get_alloc() == other.get_alloc(), "vectors are not swappable due to incompatible allocators");
    }
    ...
    size_type& get_capacity() noexcept
    {
        return m_impl.get_second().capacity;
    }

    template <bool VerifyNewCapacity, class TransferPolicy, class ConstructPolicy, class... Types>
    void grow_with_optional_check_and_adjust_resize(
        size_type new_capacity, ConstructPolicy construction_handler, TransferPolicy transfer_handler, Types&&... args
    )
    {
        if constexpr (VerifyNewCapacity)
        {
            grow<true>(new_capacity, construction_handler, transfer_handler, forward<Types>(args)...);
        }
        else
        {
            grow_unchecked<true>(new_capacity, construction_handler, transfer_handler, forward<Types>(args)...);
        }
    }

    template <bool AdjustSize, class TransferPolicy, class ConstructPolicy, class... Types>
    void
    grow(size_type new_capacity, ConstructPolicy construction_handler, TransferPolicy transfer_handler, Types&&... args)
    {
        throw_exception_if_not<length_error>(new_capacity <= max_size(), "vector is too large");
        grow_unchecked<AdjustSize>(new_capacity, construction_handler, transfer_handler, forward<Types>(args)...);
    }
    ...
    )
    {
        ...
        const size_type size_adjustment{construction_handler(alloc, new_buffer, forward<Types>(args)...)};
    ...
    {
        return []([[maybe_unused]] allocator_type& alloc, [[maybe_unused]] pointer buffer, size_t old_size)
        {
            return old_size;
        };
    }
    ...
    static constexpr auto make_construct_at_helper(size_type pos)
    {
        ...
        {
            allocator_traits_type::construct(alloc, buffer + pos, forward<decltype(args)>(args)...);
            return 1u;
        };
    }
    ...
    {
        return [pos](allocator_type& alloc, pointer buffer, auto first, size_type count)
        {
    ...
    {
        return [pos](allocator_type& alloc, pointer buffer, const Ty& value, size_t count)
        {
    ...
    {
        return [](allocator_type& alloc, pointer dst, const vector& other) noexcept
        {
    ...
    static constexpr auto make_transfer_without_shift() noexcept
    {
        ...
        {
            return [](allocator_type& alloc, pointer dst, size_type count, const pointer src)
            {
    ...
    static constexpr auto make_transfer_with_shift_right(size_type left_bound, size_type right_bound) noexcept
    {
        return [left_bound, right_bound](allocator_type& alloc, pointer dst, size_type count, const pointer src)
        {
            make_transfer_without_shift()(alloc, dst, left_bound, src);
            make_transfer_without_shift()(alloc, dst + right_bound, count - left_bound, src + left_bound);
        };
    }
    ...
    static constexpr auto make_dummy_transfer() noexcept
    {
        return []([[maybe_unused]] allocator_type& alloc,
                  [[maybe_unused]] pointer         dst,
                  [[maybe_unused]] size_type       count,
    ...
    static void deallocate_buffer(allocator_type& alc, pointer buffer, size_type obj_count) noexcept
    {
...
void swap(vector<Ty, Allocator>& lhs, vector<Ty, Allocator>& rhs) noexcept(noexcept(lhs.swap(rhs)))
{
    lhs.swap(rhs);
}

template <class Ty, class Allocator>
bool operator==(const vector<Ty, Allocator>& lhs, const vector<Ty, Allocator>& rhs)
{
...
bool operator<(const vector<Ty, Allocator>& lhs, const vector<Ty, Allocator>& rhs)
{
    return lexicographical_compare(lhs.begin(), lhs.end(), rhs.begin(), rhs.end(), less<Ty>{});
}
