...
namespace ktl
{
template <typename IntegralTy, typename Ty> static volatile IntegralTy* atomic_address_as(Ty& value) noexcept
{
...
class universal_lock : non_relocatable
{
  public:
    ...
    void lock()
    {
        ...
        {
            KeWaitForSingleObject(addressof(m_mtx), Executive, KernelMode, false, nullptr);
        }
        else
        {
...
template <class Lock> class auto_lock
{
  public:
    auto_lock(Lock& lock) noexcept : m_lock{lock}
    {
        m_lock.lock();
    }
    ~auto_lock()
...
template <memory_order order> void atomic_thread_fence() noexcept
{
    KeMemoryBarrier();
}

template <> inline void atomic_thread_fence<memory_order_relaxed>() noexcept
{
}

template <> inline void atomic_thread_fence<memory_order_seq_cst>() noexcept
{
    volatile long         guard; // Not initialized to avoid an unnecessary operation;
    [[maybe_unused]] auto value{InterlockedIncrement(atomic_address_as<long>(guard))};
}
...
template <memory_order order> void atomic_signal_fence() noexcept
{
    KeMemoryBarrierWithoutFence();
}

template <> inline void atomic_signal_fence<memory_order_relaxed>() noexcept
{
}
...
template <class Ty> Ty kill_dependency(Ty arg) noexcept
{ // "magic" template that kills
...
namespace th::details
{
template <memory_order order, typename Dummy = memory_order> struct invalid_memory_order
{
...
template <memory_order order> struct store_memory_order_checker
{
};

template <>
struct store_memory_order_checker<memory_order::consume> : invalid_memory_order<memory_order::consume>
{
};

template <>
struct store_memory_order_checker<memory_order::acquire> : invalid_memory_order<memory_order::acquire>
{
};

template <>
struct store_memory_order_checker<memory_order::acq_rel> : invalid_memory_order<memory_order::acq_rel>
{
};

template <memory_order order> struct load_memory_order_checker
{
};

template <>
struct load_memory_order_checker<memory_order::release> : invalid_memory_order<memory_order::release>
{
};

template <>
struct load_memory_order_checker<memory_order::acq_rel> : invalid_memory_order<memory_order::acq_rel>
{
};
...
{
    static_assert(is_integral_v<IntegralTy>, "tried to reinterpret memory as non-integral");
...
template <memory_order order> void make_load_barrier() noexcept
{
...
template <memory_order order> void make_store_barrier() noexcept
{
...
template <class Ty> struct atomic_padded
{
    alignas(sizeof(Ty)) mutable Ty value; // align to sizeof(T); x86 stack aligns
                                          // 8-byte objects on 4-byte boundaries
};

template <class Ty> struct atomic_storage_selector
{
    using storage_type = atomic_padded<Ty>;
    using lock_type    = universal_lock;
};

template <class Ty> struct atomic_storage_selector<Ty&>
{
    using storage_type = Ty&;
    using lock_type    = universal_lock;
};

template <class Ty, size_t = sizeof(remove_reference_t<Ty>)> struct atomic_storage;

template <class Ty, size_t /* = ... */> struct atomic_storage
{
  ...
  public:
    ...
    constexpr atomic_storage(conditional_t<is_reference_v<Ty>, Ty, const value_type> value) noexcept
        : m_value{value}
    {
        // non-atomically initialize this atomic
    }

    template <memory_order order = memory_order_seq_cst> void store(const value_type value) noexcept
    {
        guard_t guard{m_lock};
        m_value = value;
    }

    template <memory_order order = memory_order_seq_cst> [[nodiscard]] value_type load() const noexcept
    {
    ...
    template <memory_order order = memory_order_seq_cst>
    bool compare_exchange_strong(value_type& expected, const value_type desired) noexcept
    { // CAS
...
template <class Ty, class InterlockedPolicy> class interlocked_storage
{
  public:
    using value_type = Ty;
  ...
  private:
    ...
    using storage_type        = typename atomic_storage_selector<value_type>::storage_type;

  public:
    interlocked_storage() noexcept(is_nothrow_default_constructible_v<Ty>) = default;

    constexpr interlocked_storage(conditional_t<is_reference_v<Ty>, Ty, const value_type> value) noexcept
        : m_value{value}
    {
        // non-atomically initialize this atomic
    }

    template <memory_order order = memory_order_seq_cst> value_type load() const noexcept
    {
    ...
    template <memory_order order = memory_order_seq_cst> value_type exchange(const value_type value) noexcept
    {
        const auto          new_value{atomic_reinterpret_as<internal_value_type>(value)};
        internal_value_type old_value{InterlockedPolicy::exchange(get_storage(), new_value)};
    ...
    bool compare_exchange_strong(value_type& expected, const value_type desired) noexcept
    {
        const auto expected_bytes{atomic_reinterpret_as<internal_value_type>(expected)}; // read before atomic
        const auto desired_bytes{atomic_reinterpret_as<internal_value_type>(desired)};

        const auto old_value{
            InterlockedPolicy::compare_exchange_strong(get_storage(), desired_bytes, expected_bytes)
        };
  ...
  protected:
    ...
    {
        return const_cast<internal_value_type*>(atomic_address_as<const internal_value_type>(m_value));
    }
...
template <size_t> struct interlocked_policy;

template <> struct interlocked_policy<32>
{
    ...
    static long compare_exchange_strong(volatile long* place, long desired, long expected) noexcept
    {
...
#define DEFINE_INTERLOCKED_POLICY(type, bitness)                                                        \
    template <> struct interlocked_policy<bitness>                                                      \
    {                                                                                                   \
        using value_type = type;                                                                        \
                                                                                                        \
        static type exchange(volatile type* place, value_type new_value) noexcept                       \
        {                                                                                               \
            return InterlockedExchange##bitness##(place, new_value);                                    \
        }                                                                                               \
                                                                                                        \
        static type compare_exchange_strong(volatile type* place, type desired, type expected) noexcept \
        {                                                                                               \
...
#define DEFINE_ATOMIC_STORAGE(sizeoftype)                                                                      \
    template <class Ty>                                                                                        \
    struct atomic_storage<Ty, sizeoftype> : interlocked_storage<Ty, interlocked_policy<sizeoftype * CHAR_BIT>> \
    {                                                                                                          \
      public:                                                                                                  \
        using MyBase = interlocked_storage<Ty, interlocked_policy<sizeoftype * CHAR_BIT>>;                     \
                                                                                                               \
      public:                                                                                                  \
        using MyBase::MyBase;                                                                                  \
                                                                                                               \
...
template <class Ty, size_t = sizeof(Ty)> struct atomic_integral; // not defined

template <class Ty, class IntegralPolicy> class interlocked_integral : public atomic_storage<Ty>
{
  public:
  ...
  public:
    ...
    value_type fetch_add(const value_type operand) noexcept
    {
        return static_cast<value_type>(
            IntegralPolicy::add(get_storage(), static_cast<internal_value_type>(operand))
        );
    }
    ...
    {
        return static_cast<value_type>(
            IntegralPolicy::and_op(get_storage(), static_cast<internal_value_type>(operand))
        );
    }
    ...
    {
        return static_cast<value_type>(
            IntegralPolicy::or_op(get_storage(), static_cast<internal_value_type>(operand))
        );
    }
    ...
    {
        return static_cast<value_type>(
            IntegralPolicy::xor_op(get_storage(), static_cast<internal_value_type>(operand))
        );
    }

    value_type operator++() noexcept
    {
        return static_cast<value_type>(IntegralPolicy::pre_increment(get_storage()));
    }

    value_type operator++(int) noexcept
    {
        return static_cast<value_type>(IntegralPolicy::post_increment(get_storage()));
    }

    value_type operator--() noexcept
    {
        return static_cast<value_type>(IntegralPolicy::pre_decrement(get_storage()));
    }

    value_type operator--(int) noexcept
    {
        return static_cast<value_type>(IntegralPolicy::post_decrement(get_storage()));
    }
...
template <size_t> struct integral_policy;

template <> struct integral_policy<32>
{
    using value_type = long;
...
#define DEFINE_INTEGRAL_POLICY(type, bitness)                         \
    template <> struct integral_policy<bitness>                       \
    {                                                                 \
        using value_type = type;                                      \
                                                                      \
...
template <class Ty, size_t> struct integral_storage;

#define DEFINE_INTEGRAL_STORAGE(sizeoftype)                                                                    \
    template <class Ty>                                                                                        \
    struct integral_storage<Ty, sizeoftype> : interlocked_integral<Ty, integral_policy<sizeoftype * CHAR_BIT>> \
    {                                                                                                          \
      public:                                                                                                  \
        using MyBase = interlocked_integral<Ty, integral_policy<sizeoftype * CHAR_BIT>>;                       \
                                                                                                               \
      public:                                                                                                  \
        using MyBase::MyBase;                                                                                  \
                                                                                                               \
...
template <class Ty> struct is_always_lock_free
{
    using value_type = Ty;
    static constexpr size_t SIZE_OF_TYPE{sizeof(Ty)};

    static constexpr bool value = SIZE_OF_TYPE <= sizeof(uintmax_t) && (SIZE_OF_TYPE & SIZE_OF_TYPE - 1) == 0;
};

template <class Ty> inline constexpr bool is_always_lock_free_v = is_always_lock_free<Ty>::value;

template <class Ty> struct integral_facade : integral_storage<Ty, sizeof(Ty)>
{
    ...
    {
        return const_cast<integral_facade*>(this)->MyBase::fetch_add(operand, _Order);
    }
    ...
    {
        return const_cast<integral_facade*>(this)->MyBase::fetch_and(operand, _Order);
    }
    ...
    {
        return const_cast<integral_facade*>(this)->MyBase::fetch_or(operand, _Order);
    }
    ...
    {
        return const_cast<integral_facade*>(this)->MyBase::fetch_xor(operand, _Order);
    }
    ...
    {
        return static_cast<Ty>(const_cast<integral_facade*>(this)->MyBase::fetch_add(operand) + operand);
    }
    ...
    Ty operator-=(const Ty operand) volatile noexcept
    {
        return static_cast<Ty>(const_cast<integral_facade*>(this)->fetch_sub(operand) - operand);
    }
    ...
    Ty operator&=(const Ty operand) volatile noexcept
    {
        return static_cast<Ty>(const_cast<integral_facade*>(this)->MyBase::fetch_and(operand) & operand);
    }
    ...
    Ty operator|=(const Ty operand) volatile noexcept
    {
        return static_cast<Ty>(const_cast<integral_facade*>(this)->MyBase::fetch_or(operand) | operand);
    }
    ...
    Ty operator^=(const Ty operand) volatile noexcept
    {
        return static_cast<Ty>(const_cast<integral_facade*>(this)->MyBase::fetch_xor(operand) ^ operand);
    }

    [[nodiscard]] static Ty negate(const Ty value) noexcept
    { // returns two's complement negated value of
      // value
        return static_cast<Ty>(0U - static_cast<make_unsigned_t<Ty>>(value));
    }
};

template <class Ty> struct atomic_floating
{
    static_assert(always_false_v<Ty>, "not implemented");
};

template <class Ty> struct atomic_pointer;

template <class Ty> struct atomic_pointer : integral_storage<Ty, sizeof(uintptr_t)>
{
    ...
    template <memory_order order = memory_order_seq_cst> Ty fetch_add(const ptrdiff_t offset) noexcept
    {
    ...
    Ty fetch_sub(const ptrdiff_t offset) volatile noexcept
    {
        return fetch_add<order>(static_cast<ptrdiff_t>(0 - static_cast<size_t>(offset)));
    }

    template <memory_order order = memory_order_seq_cst> Ty fetch_sub(const ptrdiff_t offset) noexcept
    {
        return fetch_add(static_cast<ptrdiff_t>(0 - static_cast<size_t>(offset)));
    }
    ...
    Ty operator-=(const ptrdiff_t offset) volatile noexcept
    {
        return fetch_add(static_cast<ptrdiff_t>(0 - static_cast<size_t>(offset))) - offset;
    }
...
template <bool Cond> struct atomic_template_selector
{
    template <class Ty1, class> using type = Ty1;
};

template <> struct atomic_template_selector<false>
{
    template <class, class Ty2> using type = Ty2;
};

template <class Ty, class ValueTy = Ty> struct atomic_base_type_selector
{
    using type = typename atomic_template_selector<is_floating_point_v<Ty>>::template type<
        th::details::atomic_floating<ValueTy>,
        typename atomic_template_selector<is_integral_v<Ty> && !is_same_v<bool, Ty>>::template type<
            th::details::integral_facade<ValueTy>,
            typename atomic_template_selector<is_pointer_v<Ty> && is_object_v<remove_pointer_t<Ty>>>::
                template type<th::details::atomic_pointer<ValueTy>, th::details::atomic_storage<ValueTy>>>>;
};

template <class Ty> using atomic_base_t = typename atomic_base_type_selector<Ty>::type;

template <class Ty> class atomic : public atomic_base_t<Ty>, public non_relocatable
{ // atomic value
  private:
  ...
  public:
    static constexpr bool is_always_lock_free = th::details::is_always_lock_free_v<Ty>;

  public:
    static_assert(
        is_trivially_copyable_v<Ty> && is_copy_constructible_v<Ty> && is_move_constructible_v<Ty>
            && is_copy_assignable_v<Ty> && is_move_assignable_v<Ty>,
  ...
  public:
    ...
    {
        return th::details::is_always_lock_free_v<Ty>;
    }
    ...
    template <memory_order order> void store(const Ty value) volatile noexcept
    {
    ...
    template <memory_order order> [[nodiscard]] Ty load() const volatile noexcept
    {
    ...
    template <memory_order order> Ty exchange(const Ty value) volatile noexcept
    {
    ...
    bool compare_exchange_strong(Ty& expected, const Ty desired) volatile noexcept
    {
        return const_cast<atomic*>(this)->MyBase::compare_exchange_strong(expected, desired);
    }

    template <memory_order order>
    bool compare_exchange_strong(Ty& expected, const Ty desired) volatile noexcept
    {
        return const_cast<atomic*>(this)->MyBase::compare_exchange_strong<order>(expected, desired);
    }

    template <memory_order on_success, memory_order on_failure>
    bool compare_exchange_strong(Ty& expected, const Ty desired) volatile noexcept
    {
        return MyBase::compare_exchange_strong<combine_cas_memory_orders<on_success, on_failure>()>(
            expected, desired
        );
    }
    ...
    template <memory_order order> bool compare_exchange_weak(Ty& expected, const Ty desired) volatile noexcept
    {
        return MyBase::compare_exchange_strong<order>(expected, desired);
    }

    template <memory_order order> bool compare_exchange_weak(Ty& expected, const Ty desired) noexcept
    {
        return MyBase::compare_exchange_strong<order>(expected, desired);
    }
...
template <class Ty> atomic(Ty) -> atomic<Ty>;

template <class Ty> struct atomic_ref
{
    static_assert(always_false_v<Ty>, "not implemented");
};
...
class atomic_flag
{ // flag with test-and-set semantics
  public:
    constexpr atomic_flag() noexcept = default;

    template <memory_order order = memory_order_seq_cst> [[nodiscard]] bool test() const noexcept
    {
        return m_storage.load<order>() != false;
    }

    template <memory_order order = memory_order_seq_cst> [[nodiscard]] bool test() const volatile noexcept
    {
        return m_storage.load<order>() != false;
    }

    template <memory_order order = memory_order_seq_cst> bool test_and_set() noexcept
    {
        return m_storage.exchange<order>(true) != false;
    }

    template <memory_order order = memory_order_seq_cst> bool test_and_set() volatile noexcept
    {
        return m_storage.exchange<order>(true) != false;
    }

    template <memory_order order = memory_order_seq_cst> void clear() noexcept
    {
        m_storage.store<order>(false);
    }

    template <memory_order order = memory_order_seq_cst> void clear() volatile noexcept
    {
        m_storage.store<order>(false);
    }
...
template <memory_order order>
[[nodiscard]] inline bool atomic_flag_test(const volatile atomic_flag* const flag) noexcept
{
    return flag->test();
}

template <memory_order order>
[[nodiscard]] inline bool atomic_flag_test(const atomic_flag* const flag) noexcept
{
    return flag->test();
}

template <memory_order order>
[[nodiscard]] inline bool atomic_flag_test_explicit(const volatile atomic_flag* const flag) noexcept
{
    return flag->test();
}

template <memory_order order>
[[nodiscard]] inline bool atomic_flag_test_explicit(const atomic_flag* const flag) noexcept
{
    return flag->test();
}

template <memory_order order> inline bool atomic_flag_test_and_set(atomic_flag* const flag) noexcept
{
    return flag->test_and_set();
}

template <memory_order order> inline bool atomic_flag_test_and_set(volatile atomic_flag* const flag) noexcept
{
    return flag->test_and_set();
}

template <memory_order order> inline bool atomic_flag_test_and_set_explicit(atomic_flag* const flag) noexcept
{
    return flag->test_and_set();
}

template <memory_order order>
inline bool atomic_flag_test_and_set_explicit(volatile atomic_flag* const flag) noexcept
{
    return flag->test_and_set();
}

template <memory_order order> inline void atomic_flag_clear(atomic_flag* const flag) noexcept
{
    flag->clear();
}

template <memory_order order> inline void atomic_flag_clear(volatile atomic_flag* const flag) noexcept
{
    flag->clear();
}

template <memory_order order> inline void atomic_flag_clear_explicit(atomic_flag* const flag) noexcept
{
    flag->clear();
}

template <memory_order order>
inline void atomic_flag_clear_explicit(volatile atomic_flag* const flag) noexcept
{
    flag->clear();
}
