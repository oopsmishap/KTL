...
{
template <class SizeType> static void throw_if_index_greater_than_size(SizeType index, SizeType size)
{
    throw_exception_if<out_of_range>(index > size, "pos is out of range");
}

template <class ValueTy, class SizeType>
static decltype(auto) at_index_verified(ValueTy* data, SizeType idx, SizeType size)
{
