...
template <class Ty, typename Enable = void> struct hash;
...
{
    size_t operator()(const basic_winnt_string<CharT, BufferSize, Traits, Alloc>& str) const noexcept
    {
        return hash_array(str.data(), str.size());
    }
};

template <typename CharT, class Traits> struct hash<basic_winnt_string_view<CharT, Traits>>
{
...
template <class Ty, class Dx> struct hash<unique_ptr<Ty, Dx>>
{
...
template <class Ty> struct hash<shared_ptr<Ty>>
{
...
template <class Ty> struct hash<intrusive_ptr<Ty>>
{
...
template <typename Enum> struct hash<Enum, enable_if_t<is_enum_v<Enum>>>
{
