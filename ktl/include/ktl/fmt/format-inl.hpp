...
{
    static constexpr const int value = static_cast<int>(sizeof(T) * ktl::numeric_limits<unsigned char>::digits);
};
...
class fp
{
  ...
  public:
    ...
    // normalized form.
    static constexpr const int      double_significand_size = ktl::numeric_limits<double>::digits - 1;
...
inline digits::result grisu_gen_digits(fp value, uint64_t error, int& exp, Handler& handler)
{
    ...
    do
    {
        ...
        result         = handler.on_digit(
            static_cast<char>('0' + digit), power_of_10_64(exp) << -one.e, remainder, error, exp, true
        );
        if (result != digits::more)
            return result;
    } while (exp > 0);
...
template <int N> bool check_divisibility_and_divide_by_pow5(uint32_t& n) noexcept
{
...
template <> struct cache_accessor<float>
{
    ...
    static carrier_uint compute_mul(carrier_uint u, const cache_entry_type& cache) noexcept
    {
    ...
    static bool compute_mul_parity(carrier_uint two_f, const cache_entry_type& cache, int beta_minus_1) noexcept
    {
    ...
    static carrier_uint
    compute_left_endpoint_for_shorter_interval_case(const cache_entry_type& cache, int beta_minus_1) noexcept
    {
    ...
    static carrier_uint
    compute_right_endpoint_for_shorter_interval_case(const cache_entry_type& cache, int beta_minus_1) noexcept
    {
        return static_cast<carrier_uint>(
    ...
    static carrier_uint
    compute_round_up_for_shorter_interval_case(const cache_entry_type& cache, int beta_minus_1) noexcept
    {
        return (static_cast<carrier_uint>(cache >> (64 - float_info<float>::significand_bits - 2 - beta_minus_1))
                + 1)
             / 2;
    }
};
...
template <class T>
bool is_endpoint_integer(typename float_info<T>::carrier_uint two_f, int exponent, int minus_k) noexcept
{
...
template <class T>
bool is_center_integer(typename float_info<T>::carrier_uint two_f, int exponent, int minus_k) noexcept
{
...
template <class T> inline decimal_fp<T> shorter_interval_case(int exponent) noexcept
{
...
#ifdef FMT_FUZZ
        if (precision > 100000)
            throw ktl::runtime_error("fuzz mode - avoid large allocation inside snprintf");
#endif
...
{
    FMT_THROW((ktl::format_error{ktl::persistent_message_t{}, ktl::ansi_string_view{message}}));
}
