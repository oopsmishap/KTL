...
namespace detail
{
template <typename T> using is_exotic_char = bool_constant<!ktl::is_same<T, char>::value>;
}
...
template <> struct is_char<wchar_t> : ktl::true_type
{
};
template <> struct is_char<detail::char8_type> : ktl::true_type
{
};
template <> struct is_char<char16_t> : ktl::true_type
{
};
template <> struct is_char<char32_t> : ktl::true_type
{
};
...
template <typename T>
auto join(ktl::initializer_list<T> list, wstring_view sep) -> join_view<const T*, const T*, wchar_t>
{
    return join(ktl::begin(list), ktl::end(list), sep);
}

template <typename Char, FMT_ENABLE_IF(!ktl::is_same<Char, char>::value)>
auto vformat(basic_winnt_string_view<Char> format_str, basic_format_args<buffer_context<type_identity_t<Char>>> args)
...
template <typename S, typename... Args, typename Char = char_t<S>, FMT_ENABLE_IF(!ktl::is_same<Char, char>::value)>
...
template <
    ...
    typename Char = char_t<S>,
    FMT_ENABLE_IF(detail::is_locale<Locale>::value&& detail::is_exotic_char<Char>::value)>
inline auto format(const Locale& loc, const S& format_str, Args&&... args) -> ktl::basic_winnt_string<Char>
{
...
/**
  ...
  *T*.
 */
template <typename T> inline auto to_wstring(const T& value) -> ktl::unicode_string
{
