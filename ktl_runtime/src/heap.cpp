...
static constexpr std::align_val_t get_max_alignment_for_pool(pool_type_t pool_type)
{
...
static void* allocate_impl(const alloc_request& request) noexcept
{
    ...
    crt_assert_with_msg(alignment <= MAX_ALLOCATION_ALIGNMENT, "allocation alignment is too large");

    const size_t page_aligned_size{(max)(bytes_count, static_cast<size_t>(MAX_ALLOCATION_ALIGNMENT))};
...
template <> void* allocate_memory<OnAllocationFailure::DoNothing>(alloc_request request) noexcept
{
    return crt::allocate_impl(request);
}

template <> void* allocate_memory<OnAllocationFailure::ThrowException>(alloc_request request)
{
