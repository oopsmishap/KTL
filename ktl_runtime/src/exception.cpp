...
#include <ktl_runtime/crt_assert.hpp>
#include <ktl_runtime/exception.hpp>
...
) noexcept
{
    m_buffer = static_cast<byte*>(
        allocate_memory<OnAllocationFailure::DoNothing>(alloc_request_builder{RESERVED_BYTES_COUNT, NonPagedPool}
                                                            .set_alignment(SLOT_ALIGNMENT)
                                                            .set_pool_tag(ALLOCATION_TAG)
                                                            .build())
    );
...
void exception_allocator::deallocate(byte* ptr) noexcept
{
    ...
    if (const bool not_in_buffer{ptr < m_buffer || ptr > last_slot_begin}; not_in_buffer)
    {
        free_to_heap(ptr);
    }
    else
...
{
    auto& counter{reinterpret_cast<volatile LONG&>(m_current_slot)};
    if (const auto slot_index = static_cast<size_t>(InterlockedIncrement(&counter) - 1); slot_index < SLOT_COUNT)
    {
...
byte* exception_allocator::allocate_from_heap(size_t bytes_count)
{
    ...
    void* const buffer{
        allocate_memory<OnAllocationFailure::ThrowException>(alloc_request_builder{bytes_count, NonPagedPool}
                                                                 .set_alignment(SLOT_ALIGNMENT)
                                                                 .set_pool_tag(ALLOCATION_TAG)
                                                                 .build())
    };
...
{
    crt_assert_with_msg(
        get_current_irql() <= DISPATCH_LEVEL, "Deallocating to heap is disabled at IRQL > DISPATCH_LEVEL"
    );
    deallocate_memory(
        free_request_builder{ptr, SLOT_SIZE}.set_alignment(SLOT_ALIGNMENT).set_pool_tag(ALLOCATION_TAG).build()
    );
}
...
exception_base::exception_base(const char* msg, size_t msg_length) : m_data{make_shared_data(msg, msg_length)}
{
}
...
exception_base::exception_base(const exception_base& other) noexcept : m_data{other.m_data}
{
    if (is_shared())
    {
...
exception_base::exception_base(exception_base&& other) noexcept : m_data{other.m_data}
{
    other.m_data = "";
}

exception_base& exception_base::operator=(const exception_base& other) noexcept
{
    if (this != &other)
    {
...
auto exception_base::construct_from_unicode(const wchar_t* msg, size_t length) -> masked_storage_t
{
    if (get_current_irql() >= DISPATCH_LEVEL)
    {
...
{
    return is_shared() ? get_shared()->description : static_cast<const char*>(m_data);
}
...
auto exception_base::make_shared_data(const char* msg, size_t msg_length) -> masked_ptr_t
{
    byte* buffer{details::exc_memory_pool.allocate(sizeof(shared_data) + msg_length + 1ull)};
...
auto exception_base::make_shared_data(const wchar_t* msg, size_t msg_length) -> masked_ptr_t
{
    unsigned long description_length;
    const auto    src_length_in_bytes{static_cast<unsigned long>(msg_length * sizeof(wchar_t))};
    RtlUnicodeToMultiByteSize(&description_length, msg, src_length_in_bytes);
    byte* buffer{details::exc_memory_pool.allocate(sizeof(shared_data) + description_length + 1u)};
    auto* msg_buf{reinterpret_cast<char*>(buffer + sizeof(shared_data))};
    RtlUnicodeToMultiByteN(msg_buf, description_length, nullptr, msg, src_length_in_bytes);
    return construct_header(buffer, msg_buf, src_length_in_bytes);
}

auto exception_base::construct_header(byte* buffer, char* saved_msg, size_t msg_length) noexcept -> masked_ptr_t
{
    saved_msg[msg_length] = static_cast<char>(0);
    auto* data{new (reinterpret_cast<shared_data*>(buffer)) shared_data{saved_msg, 1}};
    return mask(data);
}
