...
#include <ktl_runtime/crt_runtime.hpp>

#include <ktl_runtime/intrinsic.hpp>
...
{ // Calling constructors of
  ...
  // dynamic initialization
    ...
    for (global_handler_t* ctor_ptr = __cxx_ctors_begin__; ctor_ptr < __cxx_ctors_end__; ++ctor_ptr)
    {
...
void CRTCALL construct_array(
    void* arr_begin, size_t element_size, size_t count, object_handler_t constructor, object_handler_t destructor
)
{
...
void CRTCALL destroy_array_in_reversed_order(void* arr_begin, size_t element_size, size_t count, object_handler_t destructor)
{
...
int CRTCALL atexit(ktl::crt::global_handler_t destructor)
{
    ...
    if (auto& dtor_count = details::destructor_count; destructor)
    {
        ...
        if (const bool registration_allowed = idx < details::MAX_DESTRUCTOR_COUNT; registration_allowed)
        {
            details::destructor_stack[idx] = destructor;
            return 0;
        }
        crt_assert_with_msg(false, "registered too many static objects with non-trivial destructors");
    }
    return 1;
}
