...
#include <ktl_runtime/limits.hpp>
...
namespace details
{
[[nodiscard]] intmax_t performance_counter_info::get_frequency() const noexcept
{
    return m_frequency;
}
...
intmax_t query_system_time()
{
    const auto     user_shared_data{reinterpret_cast<const KUSER_SHARED_DATA*>(KI_USER_SHARED_DATA)};
    const auto&    system_time{user_shared_data->SystemTime};
    const uint32_t low_part{ReadNoFence(reinterpret_cast<const volatile LONG*>(&system_time.LowPart))};
    return static_cast<intmax_t>(ReadNoFence(&system_time.High1Time)) << sizeof(uint32_t) * CHAR_BIT | low_part;
}
...
{
    const auto user_shared_data{reinterpret_cast<const KUSER_SHARED_DATA*>(KI_USER_SHARED_DATA)};
    const auto raw_system_time{reinterpret_cast<const volatile LONG64*>(&user_shared_data->SystemTime)};
    /*
