...
namespace crt
{
termination_context set_termination_context(const termination_context& bsod) noexcept
{
...
void verify_seh(NTSTATUS code, const void* addr, uint32_t flags) noexcept
{
    ...
    if (const bool unwinding =
            flag_set<exceptions::win::ExceptionFlag>{flags}.has_any_of(exceptions::win::ExceptionFlag::Unwinding);
        unwinding)
    {
        set_termination_context(
            {BugCheckReason::UnwindingNonCxxFrame, code, reinterpret_cast<bugcheck_arg_t>(addr)}
        );
        terminate();
    }
}

void verify_seh_in_cxx_handler(
    NTSTATUS code, const void* addr, uint32_t flags, uint32_t unwind_info, const void* image_base
) noexcept
{
    DbgPrintEx(
        DPFLTR_DEFAULT_ID,
    ...
    {
        set_termination_context(
            {BugCheckReason::UnwindingNonCxxFrame,
             code,
...
terminate_handler_t termination_dispatcher::get_handler() const noexcept
{
    ...
    const auto result{InterlockedCompareExchangePointer(handler_ptr, nullptr, nullptr)};
    return reinterpret_cast<terminate_handler_t>(result);
}

terminate_handler_t termination_dispatcher::set_handler(terminate_handler_t handler) noexcept
{
    auto*      as_volatile{reinterpret_cast<const volatile PVOID*>(&m_handler)};
    auto*      handler_ptr{const_cast<volatile PVOID*>(as_volatile)};
...
termination_context termination_dispatcher::set_context(const termination_context& bsod) noexcept
{
    acquire_lock();
...
bugcheck_code_t termination_dispatcher::to_bugcheck_code(BugCheckReason reason) noexcept
{
