...
template <typename Ty> static relative_virtual_address<Ty> read_rva(const uint8_t** data) noexcept
{
...
static void
load_exception_info(fh4::exc_info& eh_info, const uint8_t* data, const byte* image_base, const function& fn) noexcept
{
    ...
    if (flags.has_any_of(fh4::Attributes::IsCatchFunclet))
        eh_info.primary_frame_ptr = relative_virtual_address<byte*>(read_unsigned(&data));
}

static int32_t lookup_region(
    const fh4::exc_info* eh_info, const byte* image_base, relative_virtual_address<const byte> fn, const byte* control_pc
) noexcept
{
    if (!eh_info->regions)
        return -1;
...
) noexcept
{
    ...
    {
        set_termination_context({BugCheckReason::CorruptedEhUnwindData, initial_state, unwind_node_count});
        terminate();
    }
    ...
    for (;;)
    {
        ...
        if (!target_offset)
        {
            set_termination_context(
                {BugCheckReason::CorruptedEhUnwindData,
                 initial_state,
                 unwind_node_count,
        ...
        const auto edge_type{static_cast<fh4::unwind_edge::Type>(target_offset_and_type & 3)};
        switch (edge_type)
        {
        ...
        {
            const auto destroy_fn{image_base + read_rva<void(byte*)>(&unwind_entry)};
        ...
        {
            const auto destroy_fn{image_base + read_rva<void(byte*)>(&unwind_entry)};
            byte*      obj{*reinterpret_cast<byte**>(frame_ptr + read_unsigned(&unwind_entry))};
            destroy_fn(obj);
            break;
        }
        case fh4::unwind_edge::Type::Function:
        {
            const auto destroy_fn{image_base + read_rva<void(void*, byte*)>(&unwind_entry)};
...
static win::ExceptionDisposition frame_handler(
    win::exception_record*, byte* frame_ptr, win::x64_cpu_context*, dispatcher_context* dispatcher_context
) noexcept
{
    ...
    const auto*    handler_data{static_cast<const fh4::gs4_data*>(dispatcher_context->extra_data)};
    const uint8_t* compressed_data{image_base + handler_data->func_info};

    fh4::exc_info eh_info{};
    load_exception_info(eh_info, compressed_data, image_base, *dispatcher_context->fn);
    ...
    {
        initial_state = lookup_region(&eh_info, image_base, dispatcher_context->fn->begin, throw_frame->mach.rip);
        ...
        for (uint32_t try_block_idx = 0; try_block_idx != try_block_count; ++try_block_idx)
        {
            ...
            for (uint32_t handler_idx = 0; handler_idx != handler_count; ++handler_idx)
            {
                ...
                relative_virtual_address<type_info const> type =
                    handler_flags.has_any_of(fh4::CatchBlockFlag::HasTypeInfo) ? read_rva<type_info const>(&q)
                                                                               : 0;
                ...
                if (handler_flags.has_any_of(fh4::CatchBlockFlag::ImageRva))
                {
                    for (uint32_t k = 0; k != continuation_addr_count; ++k)
                        catch_info.continuation_address[k] = image_base + read_rva<const byte>(&q);
                }
                else
                {
...
EXTERN_C win::ExceptionDisposition __CxxFrameHandler4(
    win::exception_record* exception_record, byte* frame_ptr, win::x64_cpu_context* cpu_ctx, dispatcher_context* dispatcher_ctx
) noexcept
{
    if (exception_record != &win::exc_record_cookie)
    {
        verify_seh_in_cxx_handler(
...
fh4::unwind_edge fh4::unwind_edge::read(const uint8_t** p) noexcept
{
    ...
    result.object        = relative_virtual_address<void>(
        type == Type::ObjectOffset || type == Type::ObjectPtrOffset ? read_unsigned(p) : 0
    );
