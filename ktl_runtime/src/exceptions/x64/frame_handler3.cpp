...
) noexcept
{
    ...
    auto* ctx{reinterpret_cast<x64::fh3::function_context*>(catch_info.unwind_context)};

    const byte* image_base = dispatcher_context->pdata->image_base();

    const auto eh_info_rva{
        *static_cast<const relative_virtual_address<const x64::function_eh_info>*>(dispatcher_context->extra_data)
    };
    ...
    if (home_block_index == -1 && !target_catch_handler && eh_info->eh_flags.has_any_of(EhFlag::IsNoexcept))
    {
        set_termination_context(
            {BugCheckReason::NoexceptViolation,
             home_block_index,
    ...
    {
        set_termination_context({BugCheckReason::CorruptedFunctionUnwindState, target_state, funclet_low_state});
        terminate();
    }
...
EXTERN_C win::ExceptionDisposition __GSHandlerCheck_EH(
    win::exception_record* exception_record, byte* frame_ptr, win::x64_cpu_context* cpu_ctx, dispatcher_context* ctx
) noexcept
{
