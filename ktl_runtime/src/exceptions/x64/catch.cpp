...
#include <ktl_runtime/exceptions/x64/catch.hpp>
#include <ktl_runtime/exceptions/seh.hpp>
#include <ktl_runtime/exceptions/x64/throw.hpp>

EXTERN_C ktl::crt::exceptions::symbol __ImageBase;
...
static void copy_to_catch_block(
    const byte* image_base, const catchable_type* catchable, void* catch_var, void* exception_object
) noexcept
{
    ...
    {
        const auto address{catchable->offset.apply(reinterpret_cast<uintptr_t>(exception_object))};
...
static void transfer_to_catch_block(
    const byte* image_base, flag_set<CatchFlag> adjectives, const catchable_type* catchable, void* catch_var, void* exception_object
) noexcept
{
...
static bool process_catch_block_unchecked(
    ...
    const catchable_type_list* catchable_list
) noexcept
{
...
) noexcept
{
     ...
     */
    if (throw_info.attributes.bit_intersection<ThrowFlag::IsConst, ThrowFlag::IsVolatile, ThrowFlag::IsUnaligned>()
        & adjectives.bit_negation())
    {
        return false;
    }

    return process_catch_block_unchecked(
        image_base, adjectives, match_type, catch_var, exception_object, image_base + throw_info.catchables
    );
}

EXTERN_C win::ExceptionDisposition
         __cxx_seh_frame_handler(win::exception_record* exception_record, byte*, win::x64_cpu_context*, void*)
{
    if (exception_record != &win::exc_record_cookie)
    {
        verify_seh(exception_record->code, exception_record->address, exception_record->flags.value());
    }
...
EXTERN_C win::ExceptionDisposition __cxx_call_catch_frame_handler(
    win::exception_record* exception_record, byte* frame_ptr, win::x64_cpu_context*, void* dispatcher_ctx
)
{
    if (exception_record != &win::exc_record_cookie)
    {
...
frame_handler get_frame_handler(const byte* image_base, const unwind_info& info, size_t unwind_node_count) noexcept
{
    /*
    ...
    const auto* exception_handler_and_data{reinterpret_cast<const byte*>(info.data + unwind_node_count)};
...
const unwind_info* execute_handler(dispatcher_context& ctx, frame_walk_context& cpu_ctx, machine_frame& mach) noexcept
{
    ...
    constexpr flag_set<HandlerInfo> handler_mask{HandlerInfo::Exception, HandlerInfo::Unwind};
    if (const flag_set<HandlerInfo> flags{unwind_info->flags}; flags & handler_mask)
    {

        // The number of active slots is always odd
        const auto unwind_slots = (static_cast<size_t>(unwind_info->code_count) + 1ull) & ~1ull;

        auto [handler, extra_data]{get_frame_handler(image_base, *unwind_info, unwind_slots)};
        crt_assert_with_msg(handler, "C++ exception must have an associated handler");
        ctx.extra_data = extra_data;

        auto* frame_ptr{
            reinterpret_cast<byte*>(unwind_info->frame_reg ? cpu_ctx.gp(unwind_info->frame_reg) : mach.rsp)
        };

        prepare_for_non_cxx_handler(ctx, cpu_ctx, mach);

        [[maybe_unused]] auto exc_action{
            handler(&win::exc_record_cookie, frame_ptr, reinterpret_cast<win::x64_cpu_context*>(&cpu_ctx), &ctx)
        };
    }
    return unwind_info;
}
...
EXTERN_C const byte*
__cxx_dispatch_exception(void* exception_object, const throw_info* throw_info, throw_frame& frame) noexcept
{
