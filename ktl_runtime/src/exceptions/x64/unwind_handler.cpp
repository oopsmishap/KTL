...
#include <ktl_runtime/exceptions/x64/cpu_context.hpp>
...
namespace pe
{
template <typename Ty> struct dir_header
{
...
const function* frame_walk_pdata::find_function_entry(const byte* addr) const noexcept
{
    if (!contains_address(addr))
    {
        set_termination_context(
            {BugCheckReason::NoMatchingExceptionHandler, reinterpret_cast<bugcheck_arg_t>(addr)}
        );
        terminate();
    }
...
frame_walk_pdata::frame_walk_pdata(const byte* image_base) noexcept : m_image_base(image_base)
{
    ...
    do
    {
        ...
        BREAK_IF_FALSE(pe_hdr->headers_size >= dos_hdr->image_header.value() + sizeof(pe::extended_header));
...
template <class Ty> Ty get_unwind_data_as(const unwind_info& unwind_info, uint32_t idx) noexcept
{
...
void frame_walk_pdata::unwind(const unwind_info& unwind_info, frame_walk_context& ctx, machine_frame& mach) noexcept
{
    ...
    for (uint32_t idx = 0; idx < unwind_info.code_count; ++idx)
    {
        ...
        case UnwindCode::SaveFarNonVolatileReg:
            ctx.gp(entry.info) =
                *reinterpret_cast<const uint64_t*>(mach.rsp + get_unwind_data_as<uint32_t>(unwind_info, +idx));
            idx += 2;
            break;
        ...
        case UnwindCode::SaveXmm128:
            get_xmm(ctx, entry.info) =
                *reinterpret_cast<const xmm_register*>(mach.rsp + unwind_info.data[++idx] * 16);
            break;

        case UnwindCode::SaveFarXmm128:
            get_xmm(ctx, entry.info) =
                *reinterpret_cast<const xmm_register*>(mach.rsp + unwind_info.data[idx + 1]);
            idx += 2;
            break;
