...
{
    [[maybe_unused]] const bool registered{preload_initializer_registry::get_instance().add(*this)};
    crt_assert_with_msg(registered, "Registration of preload_initializer failed");
}

preload_initializer_registry& preload_initializer_registry::get_instance() noexcept
{
...
NTSTATUS preload_initializer_registry::run_all(DRIVER_OBJECT& driver_object, UNICODE_STRING& registry_path) noexcept
{
    m_fired = true;
    for (uint32_t idx = 0; idx < m_size; ++idx)
    {
        auto* entry{m_initializers[idx]};
        if (const NTSTATUS result = entry->run(driver_object, registry_path); !NT_SUCCESS(result))
        {
            return result;
        }
    }
