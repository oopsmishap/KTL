...
namespace ktl
{
template <typename CharT, typename IntT> struct char_traits_base
{
    using char_type = CharT;
    using int_type  = IntT;

    static char_type* assign(char_type* dst, size_t count, const char_type& ch) noexcept
    {
        while (count--)
        {
            *dst++ = ch;
    ...
    [[nodiscard]] static constexpr bool eq(const char_type& lhs, const char_type& rhs) noexcept
    {
        return lhs == rhs;
    }

    [[nodiscard]] static constexpr bool lt(const char_type& lhs, const char_type& rhs) noexcept
    {
        return lhs < rhs;
    }
    ...
    {
        return static_cast<char_type*>(memmove(dst, src, count * sizeof(char_type)));
    }
    ...
    [[nodiscard]] static constexpr int compare(const char_type* str1, const char_type* str2, size_t count) noexcept
    {
    ...
    [[nodiscard]] static constexpr const char_type* find(const char_type* str, size_t count, const char_type& ch) noexcept
    {
    ...
    [[nodiscard]] static constexpr char_type to_char_type(const int_type& meta) noexcept
    {
        return static_cast<char_type>(meta);
    }

    [[nodiscard]] static constexpr int_type to_int_type(const char_type& ch) noexcept
    {
        return static_cast<int_type>(ch);
    }

    [[nodiscard]] static constexpr bool eq_int_type(const int_type& lhs, const int_type& rhs) noexcept
    {
        return lhs == rhs;
    }
    ...
    [[nodiscard]] static constexpr int_type not_eof(const int_type& meta) noexcept
    {
...
{
template <typename Elem> struct narrow_char_traits : char_traits_base<Elem, int>
{ // 1-byte types: char, char8_t, etc.
    ...
    static constexpr int compare(const char_type* str1, const char_type* str2, size_t count) noexcept
    {
    ...
    [[nodiscard]] static constexpr const char_type* find(const char_type* str, size_t count, char_type ch) noexcept
    {
        // This check is required for char8_t
        if constexpr (is_same_v<char_type, char>)
        {
    ...
    [[nodiscard]] static constexpr char_type to_char_type(int_type meta) noexcept
    {
        return static_cast<char_type>(meta);
    }
...
template <typename Elem> struct wide_char_traits : char_traits_base<Elem, wint_t>
{ // 2-byte types: wchar_t,
  // char16_t, etc.
...
template <> struct char_traits<char> : str::details::narrow_char_traits<char>
{
};
// template <>
// struct char_traits<char8_t> : str::details::narrow_char_traits<char8_t> {};

template <> struct char_traits<wchar_t> : str::details::wide_char_traits<wchar_t>
{
};
template <> struct char_traits<char16_t> : str::details::wide_char_traits<char16_t>
{
};
} // namespace ktl