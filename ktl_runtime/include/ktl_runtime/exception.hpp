...
class exception_allocator : preload_initializer
{
    ...
    static constexpr size_t RESERVED_BYTES_COUNT{PAGE_SIZE * RESERVED_PAGES_COUNT}; //!< Reserved memory amount in bytes
...
class exception_base
{
    ...
    static constexpr auto UNICODE_CONVERSION_FAILED{"(unable to convert provided Unicode description to ANSI)"};
  ...
  protected:
    ...
    template <size_t N> explicit constexpr exception_base(const char (&msg)[N]) noexcept : m_data{msg}
    {
    }

    explicit constexpr exception_base(persistent_message_t, const char* msg) noexcept : m_data{msg}
    {
    }
  ...
  private:
    ...
    static masked_storage_t construct_from_unicode(const wchar_t* msg, size_t length);
    ...
    static masked_ptr_t     construct_header(byte* buffer, char* saved_msg, size_t msg_length) noexcept;
...
struct bad_array_new_length : bad_alloc
{
    ...
    constexpr bad_array_new_length() noexcept : MyBase{"array is too long for operator[]"}
    {
    }
