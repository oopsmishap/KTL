...
{
template <typename Ty, typename IntegralTy> Ty convert_narrow(IntegralTy value) noexcept
{
    crt_assert((numeric_limits<Ty>::min)() <= value && value <= (numeric_limits<Ty>::max)());
    return static_cast<Ty>(value);
}

template <typename Ty> struct relative_virtual_address
{
    constexpr relative_virtual_address() = default;

    constexpr relative_virtual_address(offset_t offset) noexcept : m_offset{offset}
    {
    }

    relative_virtual_address(Ty* ptr, const void* base) noexcept : m_offset{calculate_offset(ptr, base)}
    {
    }
    ...
    constexpr offset_t value() const
    {
        return m_offset;
    }
    ...
    template <typename OtherTy, enable_if_t<is_convertible_v<OtherTy*, Ty*>, int> = 0>
    ...
    constexpr friend const byte* operator-(Ty* ptr, relative_virtual_address<Ty> rva) noexcept
    {
        return reinterpret_cast<const byte*>(reinterpret_cast<uintptr_t>(ptr) - rva.value());
    }

    constexpr friend relative_virtual_address operator+(relative_virtual_address lhs, offset_t rhs) noexcept
    {
    ...
    {
        return convert_narrow<offset_t>(reinterpret_cast<uintptr_t>(ptr) - reinterpret_cast<uintptr_t>(base));
    }
...
template <typename Ty> relative_virtual_address<Ty> make_rva(Ty* ptr, const void* base) noexcept
{
