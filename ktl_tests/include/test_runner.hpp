#pragma once
#include "test_container_traits.hpp"

#include <ktl/bugcheck.hpp>
...
{
    template <typename ParseContext> constexpr auto parse(ParseContext& ctx) -> decltype(ktl::begin(ctx))
    {
        auto       first{ktl::begin(ctx)};
        const auto last{ktl::end(ctx)};
        const bool invalid_format{first != last && *first != '}'};
        ktl::throw_exception_if<ktl::format_error>(invalid_format, "invalid format");
        return first;
    }
};
} // namespace tests::details
...
#define VALUE_CONTAINER_FORMATTER(char_type, prefix)                                                 \
    ...
    struct formatter<Container, char_type, ktl::enable_if_t<tests::is_value_container_v<Container>>> \
        : tests::details::basic_container_formatter                                                  \
    {                                                                                                \
        template <class FormatContext>                                                               \
        auto format(const Container& cont, FormatContext& ctx) -> decltype(ctx.out())                \
        {                                                                                            \
...
#define KV_CONTAINER_FORMATTER(char_type, prefix)                                                 \
    template <class Container>                                                                    \
    struct formatter<Container, char_type, ktl::enable_if_t<tests::is_kv_container_v<Container>>> \
        : tests::details::basic_container_formatter                                               \
    {                                                                                             \
        template <class FormatContext>                                                            \
        auto format(const Container& cont, FormatContext& ctx) -> decltype(ctx.out())             \
        {                                                                                         \
            ...
            {                                                                                     \
                if (!first)                                                                       \
                {                                                                                 \
                    out = ktl::format_to(out, FMT_COMPILE(prefix##", {}: {}"), key, value);       \
                }                                                                                 \
                else                                                                              \
                {                                                                                 \
                    first = false;                                                                \
...
ktl::ansi_string_non_paged format_non_paged(ktl::ansi_string_view format, const Types&... args)
{
...
ktl::ansi_string_non_paged format_non_paged(const char (&format)[N], const Types&... args)
{
    return format_non_paged(ktl::ansi_string_view(format), args...);
}

template <class... Types> void print(ktl::ansi_string_view format, const Types&... args)
{
    const auto msg{format_non_paged(format, args...)};
    print_impl(msg);
}

template <size_t N, class... Types> void print(const char (&format)[N], const Types&... args)
{
    print(ktl::ansi_string_view(format), args...);
}
} // namespace details

template <class Ty, class U> void check_equal(const Ty& lhs, const U& rhs, ktl::ansi_string_view hint = {})
{
    if (!(lhs == rhs))
    {
        ...
        if (!hint.empty())
        {
            msg.reserve(static_cast<typename decltype(msg)::size_type>(ktl::size(msg) + ktl::size(hint)));
...
{
  public:
    template <class TestFunc> NTSTATUS execute(TestFunc fn, ktl::ansi_string_view test_name)
    {
...
#define ASSERT_VALUE(x)                                                                                   \
    {                                                                                                     \
        const auto hint{tests::details::format_non_paged("{} is false, {}: {}", #x, __FILE__, __LINE__)}; \
        tests::check((x), hint);                                                                          \
    }

#define ASSERT_EQ(x, y)                                                                                    \
    {                                                                                                      \
        const auto hint{tests::details::format_non_paged("{} != {}, {}: {}", #x, #y, __FILE__, __LINE__)}; \
