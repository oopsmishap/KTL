    ...
    : non_relocatable
{
    ...
    {
        memcpy(data, s, REFERENCE_STR_LENGTH);
    }
    ~String() noexcept
    {
        memset(data, 0, REFERENCE_STR_LENGTH);
    }
...
struct StringInitializer
{
    ...
    StringInitializer() noexcept
    {
        g_reference_str = REFERENCE_STR;
    }
};
...
static_assert(size(StringInitializer::REFERENCE_STR) == REFERENCE_STR_LENGTH + 1);
