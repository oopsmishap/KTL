...
namespace details
{
template <class FloatingPoint, class ExpectedTy, class BinaryOp, enable_if_t<is_floating_point_v<FloatingPoint>, int> = 0>
void arithmetic_checker(FloatingPoint lhs, FloatingPoint rhs, ExpectedTy expected, BinaryOp op)
{
...
void perform_arithmetic_operations()
{
    ...
    const auto enabled_features{
        RtlGetEnabledExtendedFeatures(XSTATE_MASK_LEGACY | XSTATE_MASK_AVX | XSTATE_MASK_AVX512)
    };
    throw_exception_if_not<runtime_error>(
        enabled_features & XSTATE_MASK_LEGACY, "floating point operations are unsupported or disallowed by OS"
    );

    NTSTATUS status{KeSaveExtendedProcessorState(enabled_features, addressof(state))};

    throw_exception_if_not<kernel_error>(NT_SUCCESS(status), status, "unable to save state of the FP-coprocessor");
    ...
    throw_exception_if_not<kernel_error>(
        NT_SUCCESS(status), status, "floating point operation failed with SEH exception thrown"
    );
}
} // namespace tests::floating_point
