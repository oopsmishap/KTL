...
template <size_t N, class Exc> class ThrowAfter : public non_relocatable
{
  public:
    template <class... Types> ThrowAfter(Types&&... exc_args)
    {
...
struct TestException : runtime_error
{
    ...
    TestException() : MyBase("")
    {
    }

    template <class Ty, enable_if_t<is_constructible_v<MyBase, const Ty&>, int> = 0>
...
namespace details
{
template <class Ty> void do_call(size_t counter);

template <class Ty> void do_call_helper(size_t counter)
{
...
#define MAKE_MESSAGE(Idx) /* NOLINT(cppcoreguidelines-macro-usage)*/ CONCAT(__FUNCTION__, #Idx)
...
void throw_on_array_init()
{
    ...
    try
    {
        ...
        array<target_t, OBJECT_COUNT> arr{MAKE_MESSAGE(1), MAKE_MESSAGE(2), MAKE_MESSAGE(3), MAKE_MESSAGE(4)};
    }
    catch ([[maybe_unused]] const exception& exc)
    {
        instance_count = details::g_object_count;
...
namespace details
{
template <class Exc> NTSTATUS get_code(Exc exc)
{
...
template <class Exc> const char* get_what(Exc exc)
{
    if constexpr (!is_pointer_v<Exc>)
    {
        return exc.what();
...
template <class Exc, class ExpectedTy, class... Types> CatchBlockType catch_impl(const Types&... exc_args)
{
